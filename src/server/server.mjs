...
function resolvePath(url, clientId) {
    ...
    if (debug && !ipc.clients[clientId]) {
        ...
        if (clientId === undefined)
            console.log(
                `(DEBUG, HTTP) Could not resolve requested url ${url} (client id not found in http cookies)`
            );
        // this should never happen, but just in case...
        // it happens when importing css file from theme
        else
            console.log(
                `(DEBUG, HTTP) Requested url ${url} not resolved against session path (unregistered client id ${clientId})`
            );
    }
...
function httpRoute(req, res) {
    ...
    if (url === "/qrcode") {
        ...
        if (
            ...
            if (
                url.match(
                    /.(svg|jpg|jpeg|png|apng|gif|webp|tiff|xbm|bmp|ico|ttf|otf|woff|woff2|html|css|js)(\?[0-9]*)?$/i
                )
            ) {
                // Resolution order: session path, theme path, absolute path
                var id;
                if (req.headers.cookie) {
                    id = (req.headers.cookie.match(/client_id=([^;]+)/) ||
                        [])[1];
                }
...
server.on("error", (e) => {
    if (e.code === "EADDRINUSE") {
        console.error(
            `(ERROR, HTTP) Could not open port ${oscInPort} (already in use) `
        );
    } else {
...
http.get(
    settings.appAddresses()[0] + "/osc-ping",
    {
