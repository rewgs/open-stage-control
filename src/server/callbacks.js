...
import "./settings.js";
import "./osc"; // TODO: Make sure that this is importing just ./osc/index.js; even if so, is there a better way?
...
export function open(data, clientId) {
    // client connected
...
export function close(data, clientId) {
    // client disconnected
}

export function created(data, clientId) {
    // client created or reconnected
...
export function destroyed(data, clientId) {
    // client removed (timeout)
...
export function clipboard(data, clientId) {
    // shared clipboard
...
export function sessionAddToHistory(data) {
    var recentSessions = settings.read("recentSessions");
...
export function sessionRemoveFromHistory(data) {
    var recentSessions = settings.read("recentSessions");
    if (recentSessions.indexOf(data) > -1) {
        recentSessions.splice(recentSessions.indexOf(data), 1);
        settings.write("recentSessions", recentSessions);
...
export function sessionSetPath(data, clientId) {
    // store client's current session file path
...
export function sessionOpen(data, clientId) {
    // attempt to read session file
...
export function fragmentLoad(data, clientId) {
    // attempt to read session file
...
export function sessionOpened(data, clientId) {
    // session file opened successfully
...
export function fileRead(data, clientId, ok, callback, errorCallback) {
    // private function
...
export function fileSave(data, clientId, ok, callback) {
    // private function
    ...
    if (data.path) {
        ...
        if (!path.isAbsolute(data.path) && settings.read("remote-root"))
            data.path = path.resolve(settings.read("remote-root"), data.path);
...
export function stateOpen(data, clientId) {
    // attempt to open state file
...
export function sessionSave(data, clientId) {
    // save session file (.json)

    if (Array.isArray(data.path)) data.path = path.resolve(...data.path);

    if (!path.basename(data.path).match(/.*\.json$/))
        return console.error("(ERROR) Sessions must be saved as .json files");
...
export function stateSave(data, clientId) {
    // save state file (.json)
...
export function syncOsc(shortdata, clientId) {
    // sync osc (or midi) message with other clients

    // if widget hash is provided, look for cached data
    if (!(widgetHashTable[clientId] && widgetHashTable[clientId][shortdata.h]))
        return;
...
export function sendOsc(shortdata, clientId) {
    // send osc (or midi) message and sync with other clients
    // shortdata
    //     {
    //         h: 'widget_uuid',
    ...
    if (
        shortdata.h &&
        !(widgetHashTable[clientId] && widgetHashTable[clientId][shortdata.h])
    )
        return;
...
export function addWidget(data, clientId) {
    // cache widget osc data to reduce bandwidth usage
    // data
    //     {
    //         hash: 'widget_uuid',
...
export function removeWidget(data, clientId) {
    // clear widget osc data
...
export function errorPopup(data) {
    ipc.send("error", data);
}

export function listDir(data, clientId) {
    // remote file browser backend
