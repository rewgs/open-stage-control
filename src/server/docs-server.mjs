...
class DocsServer extends EventEmitter {
    constructor() {
        ...
        if (!fs.existsSync(path.join(this.baseUrl, "index.html"))) {
            console.error(
                "(ERROR, DOCS) Documentation assets not found in the app's directory."
            );
    ...
    start() {
        ...
        this.server.listen(0).on("listening", () => {
            ...
            console.log(
                "(INFO) Documentation server started, docs available at \n    " +
    ...
    open() {
        if (!this.started || !this.port) {
            ...
            this.on("ready", () => {
                ...
                console.log(
                    "(INFO) Opening documentation with default browser..."
                );
            });
        } else {
            open(address("http://", this.port)[0] + this.home);
    ...
    route(req, res) {
        ...
        if (!fs.existsSync(url)) {
            if (url.indexOf("index.html") !== -1) {
                return send(req, path.join(this.baseUrl, "/404.html")).pipe(
                    res
                );
            } else {
                res.writeHead(403);
