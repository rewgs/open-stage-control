...
if (
    midiBinaries[process.arch] &&
    midiBinaries[process.arch][process.platform]
) {
    var p = path.resolve(
        __dirname,
        midiBinaries[process.arch][process.platform]
...
class MidiConverter {
    constructor() {
        ...
        this.py.childProcess.on("error", (e) => {
            if (e.code === "ENOENT") {
                console.error(
                    `(ERROR, MIDI) Could not find python binary: ${e.message.replace(/spawn (.*) ENOENT/, "$1")}`
                );
            } else {
        ...
        this.py.childProcess.on("exit", (code) => {
            if (code === null)
                console.error("(ERROR, MIDI) Midi bridge process crashed");
            this.running = false;
        });
    }
    ...
    static parseIpc(message, instance) {
        ...
        try {
            ...
            if (settings.read("debug"))
                console.error(
                    `(ERROR, MIDI) Unparsed python log:\n    ${message}`
                );
        }
        if (name == "log") {
            ...
            console.error(
                `(WARNING, MIDI) binary version mismatch (${data} installed, ${midiVersion} expected)`
            );
        }
    }
    ...
    static getPythonPath() {
        var pythonPath = settings.read("midi")
            ? settings
                  .read("midi")
    ...
    static midiBinariesError() {
        console.error(
            `(ERROR, MIDI) Could not find midi binary file ${p}\nIt may have been deleted by your antivirus.`
        );
        console.log(
            "(INFO, MIDI) Falling back to python implementation (see https://openstagecontrol.ammd.net/docs/midi/midi-configuration/)."
        );
        expectMidiBinariesError = false;
    }
}
