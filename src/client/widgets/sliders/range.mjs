...
export class Range extends Fader {
    static description() {
    ...
    constructor(options) {
        ...
        super({ ...options, value: [], html: html` <canvas></canvas> ` });
        ...
        this.on("value-changed", (e) => {
            ...
            var v = [this.faders[0].getValue(), this.faders[1].getValue()];
    ...
    draginitHandle(e) {
        ...
        if (!this.touchMap[e.pointerId]) {
            ...
            for (var i in this.faders) {
                ...
                var coord =
                    this.faders[i].percentToCoord(
                        this.faders[i].valueToPercent(this.faders[i].value)
                    ) -
                    (this.getProp("horizontal") ? -1 : 1) * (i == 0 ? -20 : 20);
    ...
    draw() {
        ...
        var d = Math.round(
                fader.percentToCoord(
                    fader.valueToPercent(
                        this.faders[this.getProp("horizontal") ? 0 : 1].value
                    )
                )
            ),
            d2 = Math.round(
                fader.percentToCoord(
                    fader.valueToPercent(
                        this.faders[this.getProp("horizontal") ? 1 : 0].value
                    )
                )
            ),
