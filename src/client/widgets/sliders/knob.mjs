...
export class Knob extends Slider {
        ...
        var defaults = super.defaults(Knob).extend({
            style: {
                ...
                pips: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to show the scale's breakpoints"
                },
                dashed: {
                    type: "boolean|array",
                    value: false,
                ...
                angle: {
                    type: "number",
                    value: 270,
                    help: "Defines the angle's width of the knob, in degrees"
                }
            },
            class_specific: {
                mode: {
                    type: "string",
                    value: "vertical",
                    choices: ["vertical", "circular", "snap", "snap-alt"],
                    help: [
                ...
                sensitivity: {
                    type: "number",
                    value: 1,
                    help: "Defines the knob's sensitivity when `mode` is not `snap` "
                },
                steps: {
                ...
                origin: {
                    type: "number",
                    value: "auto",
                    help: "Defines the starting point's value of the knob's gauge"
                }
            }
        });
    ...
    draginitHandle(e) {
        ...
        if (
            this.getProp("mode") === "snap" ||
            this.getProp("mode") === "snap-alt"
        ) {
            this.percent = this.angleToPercent(
                this.coordsToAngle(e.offsetX, e.offsetY)
            );
    ...
    dragHandle(e) {
        ...
        if (this.getProp("mode") === "vertical" && !e.traversing) {
            ...
            this.percent =
                (-100 *
                    ((e.movementY / inertia) * this.getProp("sensitivity"))) /
                    this.minDimension +
                this.percent;
        } else {
            // snap or circular
            ...
            if (this.getProp("mode") === "snap-alt") {
                this.percent = this.angleToPercent(
                    this.coordsToAngle(offsetX, offsetY)
                );
            } else {
                var diff =
                    this.angleToPercent(
                    ...
                    this.angleToPercent(
                        this.coordsToAngle(this.lastOffsetX, this.lastOffsetY),
                        true
                    );
                if (Math.abs(diff) < 50 && diff !== 0)
                    this.percent +=
                        (((diff * 360) / this.maxAngle) *
    ...
    angleToPercent(angle, ignoreMaxAngle = false) {
        return ignoreMaxAngle
            ...
            : (clip(angle - (360 - this.maxAngle) / 2, [0, this.maxAngle]) /
                  this.maxAngle) *
                  100;
    }
    ...
    percentToAngle(percent) {
        ...
        return (
            ((2 * Math.PI * percent) / 100) * (this.maxAngle / 360) + // angle relative to maxAngle
            Math.PI / 2 + // quarter circle offset
            Math.PI * (1 - this.maxAngle / 360)
        ); // centering offset depending on maxAngle
    }
    ...
    draw() {
        var percent = this.steps
            ...
            minRadius =
                this.minDimension / 6 -
                (pips ? this.fontSize * 0.75 : 0) -
                this.cssVars.lineWidth / 2,
            maxRadius =
                this.minDimension / 2 -
                (pips ? this.fontSize * 2 : 0) -
                this.cssVars.lineWidth / 2;

        if (dashed)
            dashed = Array.isArray(dashed)
        ...
        if (this.getProp("design") === "default") {
            ...
            this.ctx.lineWidth = this.cssVars.lineWidth + 0.5 * PXSCALE;
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(
                r1 * Math.cos(a) + this.width / 2,
                this.height / 2 - r1 * Math.sin(a)
            );
            this.ctx.lineTo(
                r2 * Math.cos(a) + this.width / 2,
                this.height / 2 - r2 * Math.sin(a)
            );
            this.ctx.stroke();
