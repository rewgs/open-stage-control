...
module.exports = class Encoder extends (
        ...
        var defaults = super.defaults().extend({
            ...
            class_specific: {
                ...
                ticks: {
                    type: "number",
                    value: "",
                    help: "Defines the granularity / verbosity of the encoder (number of step for a 360° arc)"
                },
                sensitivity: {
                    type: "number",
                    value: 1,
                    help: "When set between 0 and 1, reduces the encoder's verbosity"
                },
                back: {
                    type: "*",
                    value: -1,
                    help: "Defines which value is sent when rotating the encoder anticlockwise"
                },
                forth: {
                    type: "*",
                    value: 1,
                    help: "Defines which value is sent when rotating the encoder clockwise"
                },
                release: {
                    type: "number",
                    value: "",
                ...
                mode: {
                    type: "string",
                    value: "circular",
                    choices: ["circular", "snap", "vertical"],
                    help: [
    ...
    dragendHandle(e, data, traversing) {
        ...
        this.percent = 50;
        this.speed = [];
        this.setValue(this.getProp("release"), {
            sync: true,
            send: true,
            dragged: true
        });
    ...
    setPercent(percent) {
        ...
        if (
            (this.percent >= 0 &&
                this.percent < 25 &&
                this.previousPercent > 75 &&
                this.previousPercent <= 100) ||
            (this.previousPercent >= 0 &&
                this.previousPercent < 25 &&
                this.percent > 75 &&
                this.percent <= 100)
        )
            dir = !dir;
    ...
    setValue(v, options = {}) {
        if (options.fromLocal && v === this.getProp("release")) {
            ...
            !options.dragged &&
            v !== this.getProp("forth") &&
            v !== this.getProp("back") &&
            this.getProp("release") === "" &&
            v !== this.getProp("release")
        )
            return;
        ...
        if (
            !options.dragged &&
            options.widget &&
            options.widget instanceof Encoder
        ) {
        ...
        this.parsersLocalScope.angle = Math.abs(
            ((this.percent % 100) / 100) * 360
        );
