...
class Slider extends Canvas {
    constructor(options) {
        ...
        for (var i in this.rangeKeys) {
            var k = this.rangeKeys[i],
                ...
                val =
                    typeof this.getProp("range")[k] === "object" &&
                    this.getProp("range")[k] !== null
        ...
        this.originValue =
            this.getProp("origin") == "auto"
                ? this.rangeValsMin
                : clip(this.getProp("origin"), [
                      this.rangeValsMin,
                      this.rangeValsMax
                  ]);
        ...
        this.on("draginit", this.draginitHandleProxy.bind(this), {
            element: this.widget,
            multitouch: options.multitouch
        });
        this.on("drag", this.dragHandleProxy.bind(this), {
            element: this.widget,
            multitouch: options.multitouch
        });
        this.on("dragend", this.dragendHandleProxy.bind(this), {
            element: this.widget,
            multitouch: options.multitouch
        });
        this.on("wheel", this.mousewheelHandleProxy.bind(this), {
            element: this.widget
        });
    ...
    mousewheelHandle(e) {
        ...
        if (this.steps) {
            ...
            if (val !== undefined)
                this.setValue(val, { sync: true, send: true, fromLocal: true });
        } else {
            this.percent = clip(
                this.percent +
                    Math.max(increment, 10 / Math.pow(10, this.decimals + 1)) *
                        direction,
                [0, 100]
            );
    ...
    percentToValue(percent) {
        ...
        for (var i = 0; i < this.rangeKeys.length - 1; i++) {
            if (h <= this.rangeKeys[i + 1] && h >= this.rangeKeys[i]) {
                return mapToScale(
                    h,
                    [this.rangeKeys[i], this.rangeKeys[i + 1]],
                    [this.rangeVals[i], this.rangeVals[i + 1]],
                    -1,
                    this.getProp("logScale")
                );
            }
        }
    }
    ...
    valueToPercent(value) {
        for (var i = 0; i < this.rangeVals.length - 1; i++) {
            if (
                (this.rangeVals[i + 1] > this.rangeVals[i] &&
                    value <= this.rangeVals[i + 1] &&
                    value >= this.rangeVals[i]) ||
                (this.rangeVals[i + 1] < this.rangeVals[i] &&
                    value >= this.rangeVals[i + 1] &&
                    value <= this.rangeVals[i])
            ) {
    ...
    setValue(v, options = {}) {
        ...
        if (
            (options.dragged || options.fromLocal) &&
            this.value.toFixed(this.decimals) == value.toFixed(this.decimals)
        )
            options.send = false;
        ...
        if (this.steps) {
            ...
            if (!isNaN(val))
                this.value = clip(val, [this.rangeValsMin, this.rangeValsMax]);
        }
            ...
            if (steps === "auto") {
                ...
                this.steps = Array(steps)
                    ...
                    .map(
                        (x, i) =>
                            (i / (steps - 1)) *
                                (this.rangeValsMax - this.rangeValsMin) +
                            this.rangeValsMin
                    );
            } else if (
                Array.isArray(steps) &&
                steps.every((x) => typeof x === "number")
            ) {
                this.steps = steps;
            } else {
                this.steps = null;
        ...
        switch (propName) {
            ...
            case "spring":
                if (this.getProp("spring") && !this.touched)
                    this.setValue(this.getSpringValue(), { ...options });
                return;
            case "colorKnob":
            case "knobSize":
