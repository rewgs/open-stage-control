...
class Widget extends EventEmitter {
        ...
        var defaults = {
            widget: {
                lock: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to prevent modifying this widget with the editor. This will not prevent deleting the widget or moving it from a container to another."
                },
                type: { type: "string", value: "auto", help: "Widget type" },
                id: {
                    type: "string",
                    value: "auto",
                    help: "Widgets sharing the same `id` will act as clones and update each other's value(s) without sending extra osc messages (avoid doing so unless the widgets expect the exact same values)."
                },
                visible: {
                    type: "boolean",
                    value: true,
                    help: "Set to `false` to hide the widget."
                },
                interaction: {
                    type: "boolean",
                    value: true,
                    help: "Set to `false` to disable pointer interactions."
                },
            ...
            geometry: {
                ...
                expand: {
                    type: "boolean|number",
                    value: false,
                    help: "If parent's layout is `vertical` or `horizontal`, set this to `true` to stretch the widget to use available space automatically."
                }
            },
            style: {
                colorText: {
                    type: "string",
                    value: "auto",
                    help: 'Text color. Set to "auto" to inherit from parent widget.'
                },
                colorWidget: {
                    type: "string",
                    value: "auto",
                    help: 'Widget\'s default accent color. Set to "auto" to inherit from parent widget.'
                },
                colorStroke: {
                    type: "string",
                    value: "auto",
                    help: 'Stroke color. Set to "auto" to use `colorWidget`.'
                },
                colorFill: {
                    type: "string",
                    value: "auto",
                    help: 'Fill color. Set to "auto" to use `colorWidget`.'
                },
                alphaStroke: {
                    type: "number",
                    value: "auto",
                    help: "Stroke color opacity."
                },
                alphaFillOff: {
                    type: "number",
                    value: "auto",
                    help: "Fill color opacity (off)."
                },
                alphaFillOn: {
                    type: "number",
                    value: "auto",
                    help: "Fill color opacity (on)."
                },
                lineWidth: {
                    type: "number",
                    value: "auto",
                ...
                borderRadius: {
                    type: "number|string",
                    value: "auto",
                    help: "Border radius expressed as a number or a css string. This property may not work for all widgets."
                },
                padding: {
                    type: "number",
                    value: "auto",
            ...
            value: {
                value: {
                    type: "*",
                    value: "",
                    help: "Define the widget's value depending on other widget's values / properties using the advanced property syntax"
                },
                default: {
                    type: "*",
                    value: "",
                    help: "If set, the widget will be initialized with this value when the session is loaded."
                },
                linkId: {
                    type: "string|array",
                    value: "",
            ...
            osc: {
                ...
                decimals: {
                    type: "integer",
                    value: 2,
                    help: ["Defines the number of decimals to send."]
                },
                target: {
                ...
                ignoreDefaults: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to ignore the server's default targets"
                },
                bypass: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to prevent the widget from sending any osc message"
                }
            },
            scripting: {
                onCreate: {
                    ...
                    editor: "javascript",
                    help: [
                        'Script executed when the widget (and its children) is created. See <a href="https://openstagecontrol.ammd.net/docs/widgets/scripting/">documentation</a>.'
                    ]
                },
                onValue: {
                    type: "script",
                    ...
                    editor: "javascript",
                    help: [
                        'Script executed when the widget\'s value updates. See <a href="https://openstagecontrol.ammd.net/docs/widgets/scripting/">documentation</a>.'
                    ]
                }
            }
        };
    ...
    constructor(options = {}) {
        ...
        if (this.getProp("onValue")) {
            ...
            this.on("value-changed", (e) => {
                if (
                    ...
                    this.mounted &&
                    !e.options.fromEdit &&
                    e.options.script !== false
                ) {
                    this.scripts.onValue.run(
                        {
                            value: e.options.widget
                                ? e.options.widget.value
                                : this.value,
                ...
                this.scripts.onTouch.run(
                    {
                        event: {
                            ...
                            handle: multi ? e.touch[0] : undefined
                        },
                        value: this.value
                    },
                    { sync: true, send: true }
        ...
        if (
            String(this.getProp("onValue")).includes("touch") &&
            !this.getProp("onTouch")
        ) {
            this.on("touch", (e) => {
                this.scripts.onValue.run(
                    {
        ...
        this.container = html`
            <div
                class="widget ${options.props.type}-container"
                id="${this.hash}"
                data-widget="${this.hash}"
            ></div>
        `;
    ...
    created(index) {
        ...
        if (this.scripts.onCreate)
            this.scripts.onCreate.run(
                { value: this.value },
                { sync: true, send: true, onCreate: true }
            );
    }
    ...
    checkLinkedProps(propNames) {
        ...
        Object.values(this.nestedLinkedProps).forEach(
            (l) => (linkedProps = linkedProps.concat(l))
        );
        Object.values(this.linkedProps).forEach(
            (l) => (linkedProps = linkedProps.concat(l))
        );
        Object.values(this.linkedPropsValue).forEach(
            (l) => (linkedProps = linkedProps.concat(l))
        );
        Object.values(this.oscReceivers).forEach(
            (r) => (linkedProps = linkedProps.concat(r.propNames))
        );
        Object.values(this.variables).forEach(
            (r) => (linkedProps = linkedProps.concat(r.propNames))
        );
        Object.values(this.fragments).forEach(
            (l) => (linkedProps = linkedProps.concat(l))
        );
        ...
        if (
            ...
            propNames
                ...
                .some(
                    (x) =>
                        typeof x === "string" && x.match(/(OSC|@|VAR|IMPORT)\{/)
                ) ||
    ...
    createLinkedPropsBindings() {
        if (
            !this.linkedCreatedCallback &&
            Object.keys(this.linkedProps).length
        ) {
            ...
            widgetManager.on("widget-created", this.linkedCreatedCallback, {
                context: this
            });
            widgetManager.on("prop-changed", this.linkedPropChangedCallback, {
                context: this
            });
        } else if (
            this.linkedCreatedCallback &&
        ...
        if (
            !this.linkedValueChangedCallback &&
            Object.keys(this.linkedPropsValue).length
        ) {
            this.linkedValueChangedCallback = (e) => {
                this.onLinkedPropsChanged(e, "value-changed");
            };

            widgetManager.on("value-changed", this.linkedValueChangedCallback, {
                context: this
            });
        } else if (
            this.linkedPropsValueCallback &&
            !Object.keys(this.linkedPropsValue).length
        ) {
        ...
        if (
            !this.linkedFragmentCallback &&
            Object.keys(this.fragments).length
        ) {
            this.linkedFragmentCallback = (e) => {
                var { path } = e;
                if (this.fragments[path])
                    this.updateProps(this.fragments[path]);
            };

            sessionManager.on("fragment-updated", this.linkedFragmentCallback, {
                context: this
            });
        } else if (
            this.linkedFragmentCallback &&
        ...
        var selfLinkedOSCProps = (this.linkedPropsValue["this"] || []).filter(
            (i) => OSCProps.indexOf(i) > -1
        );
        this.selfLinkedOSCProps = selfLinkedOSCProps.length
            ? selfLinkedOSCProps
            : false;
    }
            ...
            for (var linksStores of [this.linkedPropsValue, this.linkedProps]) {
                for (let id in linksStores) {
                    if (linksStores[id].includes(prop)) {
                        linksStores[id].splice(
                            linksStores[id].indexOf(prop),
                            1
                        );
    ...
    resolveProp(
        ...
        if (typeof propValue == "string") {
            propValue = balancedReplace("@", "{", "}", propValue, (content) => {
                ...
                if (
                    widgets[0] === this.parent &&
                    this.parent !== widgetManager &&
                    this.parent.getProp("type") === "clone"
                )
                    widgets = [this.parent.parent];

                if (storeLinks) {
                    if (propName.indexOf("-nested") !== -1)
                        propName = propName.replace("-nested", "");

                    if (k == "value") {
                        if (!this.linkedPropsValue[id])
                            this.linkedPropsValue[id] = [];
                        if (this.linkedPropsValue[id].indexOf(propName) == -1)
                            this.linkedPropsValue[id].push(propName);
                    } else {
                        if (!this.linkedProps[id]) this.linkedProps[id] = [];
                        if (this.linkedProps[id].indexOf(propName) == -1)
                            this.linkedProps[id].push(propName);
                    }

                    if (storeLinks === "nested") {
                        if (!this.nestedLinkedProps[id])
                            this.nestedLinkedProps[id] = [];
                        if (this.nestedLinkedProps[id].indexOf(propName) == -1)
                            this.nestedLinkedProps[id].push(propName);
                    }
                }
            ...
            propValue = balancedReplace("VAR", "{", "}", propValue, (args) => {
                ...
                let [name, value] = args
                    ...
                    .map((x) => x.trim())
                    .map((x) =>
                        x.replace(/VAR_[0-9]+/g, (m) => {
                            return typeof variables[m] === "string"
                                ? variables[m]
                                : JSON.stringify(variables[m]);
                        })
                    );
                ...
                if (
                    ...
                    if (this.variables[name].propNames.length > 1) {
                        this.updateProps(
                            this.variables[name].propNames.filter(
                                (x) => x !== propName
                            )
                        );
                    }
            ...
            propValue = balancedReplace(
                ...
                (file) => {
                    ...
                    if (storeLinks) {
                        ...
                        if (!this.fragments[file].includes(propName))
                            this.fragments[file].push(propName);
                    }
                    var r;
            ...
            propValue = balancedReplace("OSC", "{", "}", propValue, (args) => {
                ...
                let [address, value, usePreArgs] = args
                    .split(",")
                    .map((x) => x.trim())
                    .map((x) =>
                        x.replace(/VAR_[0-9]+/g, (m) => {
                ...
                if (!this.oscReceivers[args]) {
                    ...
                    this.oscReceivers[args].usePreArgs =
                        usePreArgs === "false" ? false : true;
                }
            ...
            try {
                propValue = balancedReplace(
                    ...
                    (code) => {
                        ...
                        if (!this.parsers[code])
                            this.parsers[code] = vm.compile(code, defaultScope);
    ...
    updateProps(propNames, widget, options, updatedProps = []) {
                ...
                if (!this.isDynamicProp(propName)) {
                    if (this.constructor._defaults[propName] === undefined) {
                        ...
                        this.errorProp(
                            propName,
                            "@{}",
                            `a container can't use its child's properties to define non-dynamic properties.`
                        );
                        continue;
                    } else if (
                        widget === this &&
                        updatedProps.length === 1 &&
                        updatedProps[0] === "value"
                    ) {
                        this.errorProp(
                            propName,
                            "@{}",
                            `a widget can't use it's own value to define non-dynamic properties.`
                        );
                        continue;
                    } else {
                        reCreate = true;
        ...
        if (changedProps.length) {
            for (var i in changedProps) {
                this.cachedProps[changedProps[i].propName] =
                    changedProps[i].propValue;
                this.onPropChanged(
                    changedProps[i].propName,
                    options,
                    changedProps[i].oldPropValue
                );
            }
        ...
        if (options && options.fromEditor) {
            ...
            if (this.checkLinkedProps(propNames)) {
                ...
                var unNotified = propNames.filter(
                    (n) => !changedProps.includes(n)
                );
    ...
    onPropChanged(propName, options = {}, oldPropValue) {
        switch (propName) {
            ...
            case "expand": {
                ...
                let container =
                    this.parent !== widgetManager &&
                    this.parent.getProp("layout") !== "default"
                        ? this.parent.container
                        : this.container;
                resize.check(container);
                if (uiDragResize.mounted && uiDragResize.widgets.includes(this))
                    uiDragResize.updateRectangle();
                return;
            }
            ...
            case "css": {
                ...
                if (re.test(oldPropValue) || re.test(this.getProp("css"))) {
                    ...
                    resize.check(container);
                    if (
                        uiDragResize.mounted &&
                        uiDragResize.widgets.includes(this)
                    )
                        uiDragResize.updateRectangle();
                }
                return;
            ...
            case "decimals":
                this.decimals = Math.min(
                    20,
                    Math.max(this.getProp("decimals"), 0)
                );
                return;
            ...
            case "target":
                ...
                if (data.address === "auto")
                    data.address = "/" + this.getProp("id");
                if (oldData.address === "auto")
                    oldData.address = "/" + this.getProp("id");
    ...
    log(message) {
        ...
        uiConsole.log(
            "log",
            `<span class="edit-widget" data-widget="${widget.builtIn ? widget.parent.hash : widget.hash}">${id}</span>: ${message}`,
            true
        );
    }

    errorProp(name, type, error) {
        let stackline = error.stack
                ? error.stack.match(/>:([0-9]+):[0-9]+/) || ""
                : "",
            line =
                stackline.length > 1
                    ? " at line " +
                      (parseInt(stackline[1]) - (type.includes("{}") ? 1 : 2))
                    : "",
        ...
        if (
            widget.parent !== widgetManager &&
            widget.parent.getProp("type") === "clone"
        )
            widget = widget.parent;

        console._error(
            `${id}${name ? "." + name : ""} ${type} error${line}: ${error}`
        );
        console.debug(error);
        uiConsole.log(
            "error",
            `<span class="edit-widget" data-widget="${widget.builtIn ? widget.parent.hash : widget.hash}">${id}</span>${name ? "." + name : ""} ${type} error${line}: ${error}`,
            true
        );
    }
    ...
    setContainerStyles(styles = ["geometry", "css", "visibility"]) {
        if (styles.includes("geometry")) {
            ...
            for (let d of ["width", "height", "top", "left"]) {
                ...
                if (val !== undefined) {
                    ...
                    geometry =
                        parseFloat(val) == val ? parseFloat(val) + "rem" : val;
                }
                ...
                if (geometry) {
                    ...
                    if (d === "height")
                        this.container.style.minHeight = geometry;
                    absolutePos =
                        absolutePos ||
                        (geometry && (d === "top" || d === "left"));
                }
            }
        ...
        if (styles.includes("css")) {
            ...
            css = css.replace(/url\(([^)]*)\)/g, (m, url) => {
                ...
                if (!parser.protocol.match(/http|data/))
                    m = m.replace(":", "_:_");
                m = m.replace(/\\/g, "\\\\");
                return m;
            });
            ...
            if (css.includes("{")) {
                ...
                css.replace(/\{[^}]*\}/g, "").replace(/^[^@#.]*:.*/gm, (m) => {
                    unScopedCss += m[m.length - 1] === ";" ? m : m + ";";
                });

                if (scopedCss.indexOf("@keyframes") > -1)
                    scopedCss = scopedCss.replace(
                        new RegExp(prefix + "\\s+([0-9]+%|to|from)", "g"),
                        " $1"
                    );
                if (scopedCss.indexOf("&") > -1)
                    scopedCss = scopedCss.replace(
            ...
            var style =
                (unScopedCss ? prefix + "{" + unScopedCss + "}\n" : "") +
                scopedCss;
            style = style.trim();
            ...
            if (this.style && this.container.contains(this.style)) {
                ...
                this.container.insertBefore(
                    this.style,
                    this.container.childNodes[0] || null
                );
            }
...
Widget.cssVariables = [
    ...
    { js: "colorText", css: "--color-text" },
    {
        js: "padding",
        css: "--widget-padding",
        toCss: (x) => parseFloat(x) + "rem",
        toJs: (x) => parseFloat(x) * PXSCALE
    },
    {
        js: "lineWidth",
        css: "--line-width",
    ...
    },
    {
        js: "borderRadius",
        css: "--border-radius",
        toCss: (x) => (parseFloat(x) == x ? x + "rem" : x),
        toJs: (x) => parseFloat(x)
    },
    {
        js: "alphaFillOn",
        css: "--alpha-fill-on",
        toCss: (x) => parseFloat(x),
        toJs: (x) => parseFloat(x)
    },
    {
        js: "alphaFillOff",
