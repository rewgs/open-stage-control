...
// var canvasQueue = require("./queue.mjs");
    ...
    cacheCanvasStyle(style) {
        ...
        for (var data of this.constructor.cssVariables) {
            ...
            if (typeof val === "string" && val.match(/(var|calc)\(/)) {
                ...
                var dummy = html`<div
                    style="position:absolute;width:var(${data.css})"
                ></div>`;
    ...
    onPropChanged(propName, options, oldPropValue) {
        ...
        switch (propName) {
            ...
            case "knobSize":
                ...
                return;
            case "css":
                if (!/\/\*\s*noresize\s*\*\//.test(this.getProp("css")))
                    // resize.check(this.canvas, true);
                    check(this.canvas, true);
                fastdom.measure(() => {
                    this.cacheCanvasStyle();
                    fastdom.mutate(() => {
                        this.batchDraw();
