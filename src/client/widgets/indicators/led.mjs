...
export class Led extends Widget {
        ...
        return super.defaults().extend({
            ...
            class_specific: {
                mode: {
                    type: "string",
                    value: "intensity",
                    choices: ["intensity", "color"],
                    help: "Defines how value is interpreted (see `value`)"
                },
                range: {
                    type: "object",
                    value: { min: 0, max: 1 },
                ...
                alphaRange: {
                    type: "object",
                    value: { min: 0, max: 1 },
                    help: "Alpha range (if `mode` is `color`)"
                },
                logScale: {
                    type: "boolean",
                    value: false,
                    help: "If `mode` is `intensity`, set to `true` to use logarithmic scale."
                }
            },
            value: {
                value: {
    ...
    constructor(options) {
        ...
        this.setValueIntensity(
            this.getProp("mode") === "intensity"
    ...
    setValue(v, options = {}) {
        var ret =
            this.getProp("mode") === "intensity"
                ? this.setValueIntensity(v)
                : this.setValueColor(v);
            ...
            v[3] =
                v[3] === undefined
                    ...
                    : mapToScale(
                          v[3],
                          [
                              this.getProp("alphaRange").min,
                              this.getProp("alphaRange").max
                          ],
                          [0, 1],
                          -1
                      );
