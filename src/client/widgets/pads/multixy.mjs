...
export class MultiXy extends Pad {
    static description() {
        ...
        var defaults = super.defaults().extend({
            style: {
                ...
                pointSize: {
                    type: "integer",
                    value: 20,
                    help: "Defines the points' default size (may be in %)"
                },
                ephemeral: {
                    type: "boolean",
                    value: false,
                    help: "When set to `true`, the points will be drawn only while touched."
                },
                pips: {
                    type: "boolean",
                    value: true,
                    help: "Set to `false` to hide the scale"
                }
            },
            class_specific: {
                points: {
                ...
                rangeX: {
                    type: "object",
                    value: { min: 0, max: 1 },
                    help: "Defines the min and max values for the x axis (see fader)"
                },
                rangeY: {
                    type: "object",
                    value: { min: 0, max: 1 },
                    help: "Defines the min and max values for the y axis (see fader)"
                },
                logScaleX: {
                    type: "boolean|number",
                    value: false,
                    help: "Set to `true` to use logarithmic scale for the x axis. Set to `-1` for exponential scale."
                },
                logScaleY: {
                    type: "boolean|number",
                    value: false,
                    help: "Set to `true` to use logarithmic scale for the y axis. Set to `-1` for exponential scale."
                },
                stepsX: {
                    type: "number|array|string",
                    value: false,
                    help: "Defines `steps` for the x axis (see fader)"
                },
                stepsY: {
                    type: "number|array|string",
                    value: false,
                ...
                sensitivity: {
                    type: "number",
                    value: 1,
                    help: "Defines the pad's sensitivity when `snap` is `false` "
                }
            }
        });
    ...
    constructor(options) {
        ...
        this.npoints = Array.isArray(this.getProp("points"))
            ? this.getProp("points").length
            : parseInt(this.getProp("points"));
        ...
        for (let i = 0; i < this.npoints; i++) {
            this.pads[i] = new Xy({
                props: {
                    ...
                    default:
                        this.getProp("default").length === this.npoints * 2
                            ? [
                                  this.getProp("default")[i * 2],
                                  this.getProp("default")[i * 2 + 1]
                              ]
                            : "",
        ...
        this.on(
            ...
            (e) => {
                ...
                this.trigger("touch", {
                    ...
                    touch: [parseInt(this.touchMap[e.pointerId]), 0]
                });
        ...
        if (this.getProp("doubleTap")) {
            if (
                typeof this.getProp("doubleTap") === "string" &&
                this.getProp("doubleTap")[0] === "/"
            ) {
                doubleTap(
                    this,
                    () => {
                        this.sendValue({
                            v: null,
                            address: this.getProp("doubleTap")
                        });
                    },
                    { element: this.widget }
                );
                ...
                doubleTap(
                    ...
                    (e) => {
                        ...
                        for (var i in this.pads) {
                            ndiff =
                                Math.abs(e.offsetX - this.padsCoords[i][0]) +
                                Math.abs(
                                    e.offsetY -
                                        (this.padsCoords[i][1] + this.height)
                                );
                            if (ndiff < diff) {
                                id = i;
                                diff = ndiff;
                        ...
                        if (this.getProp("doubleTap") == "script") {
                            ...
                            pad.setValue(
                                [
                                    pad.faders.x.getSpringValue(),
                                    pad.faders.y.getSpringValue()
                                ],
                                {
                                    sync: true,
                                    send: true,
    ...
    getPadIdFromCoords(x, y) {
        ...
        for (var i in this.pads) {
            ...
            ndiff =
                Math.abs(x - this.padsCoords[i][0]) +
                Math.abs(y - (this.padsCoords[i][1] + this.height));
    ...
    updateHandlesPosition() {
        for (var i = 0; i < this.npoints; i++) {
            ...
            this.padsCoords[i] = [
                (clip(this.pads[i].faders.x.percent, [0, 100]) / 100) *
                    (this.width - (pointSize * 2 + 2) * PXSCALE) +
                    (pointSize + 1) * PXSCALE,
                (-clip(this.pads[i].faders.y.percent, [0, 100]) / 100) *
                    (this.height - (pointSize * 2 + 2) * PXSCALE) -
                    (pointSize + 1) * PXSCALE
            ];
        }
    }
    ...
    updatePadsAttr(options) {
        ...
        for (var i = 0; i < this.npoints; i++) {
            ...
            for (var n of [
                ...
                this.pointsAttr[i][n] =
                    a[n] === undefined ? this.getProp(n) : a[n];
                ...
                if (this.pads[i].cachedProps[n] !== this.pointsAttr[i][n]) {
                    ...
                    var cssVar = this.constructor.cssVariables.find(
                        (x) => x.js == n
                    );
                    if (cssVar) {
                        this.pads[i].canvas.style.setProperty(
                            cssVar.css,
                            this.pointsAttr[i][n] == "auto"
                                ? ""
                                : this.pointsAttr[i][n]
                        );
                    }
...
MultiXy.dynamicProps = MultiXy.prototype.constructor.dynamicProps
    .filter((n) => n !== "decimals")
    .concat(["pointsAttr"]);
