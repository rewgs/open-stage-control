...
export class Xy extends Pad {
        ...
        var defaults = super.defaults().extend({
            style: {
                ...
                pointSize: {
                    type: "integer",
                    value: 20,
                    help: "Defines the points' size (may be in %)"
                },
                ephemeral: {
                    type: "boolean",
                    value: false,
                    help: "When set to `true`, the point will be drawn only while touched."
                },
                pips: {
                    type: "boolean",
                    value: true,
                ...
                label: {
                    type: "string",
                    value: "",
                    help: "Text displayed in the handle"
                }
            },
            class_specific: {
                snap: {
    ...
    constructor(options) {
        ...
        this.faders = {
            x: new Fader({
                props: {
                    ...
                    horizontal: true,
                    default:
                        this.getProp("default").length === 2
                            ? this.getProp("default")[0]
                            : "",
        ...
        this.on(
            ...
            (e) => {
                ...
                if (axis !== "y")
                    this.faders.x.trigger("draginit", {
                        ...e,
                        stopPropagation: true
                    });
                if (axis !== "x")
                    this.faders.y.trigger("draginit", {
                        ...e,
                        stopPropagation: true
                    });
                this.dragHandle();
        ...
        this.on(
            ...
            (e) => {
                ...
                if (axis !== "" && e.shiftKey) axis = "";
                    ...
                    (e.shiftKey && axis === "") ||
                    (!e.shiftKey && axis === "auto")
                ) {
                    if (this.autoAxisLock === "") {
                        if (Math.abs(e.movementX) > Math.abs(e.movementY)) {
                            this.autoAxisLock = "x";
                        } else if (
                            Math.abs(e.movementY) > Math.abs(e.movementX)
                        ) {
                            this.autoAxisLock = "y";
                        }
                    }
        ...
        this.on(
            ...
            (e) => {
                ...
                if (axis !== "" && e.shiftKey) axis = "";
                if (axis !== "y")
                    this.faders.x.trigger("dragend", {
                        ...e,
                        stopPropagation: true
                    });
                if (axis !== "x")
                    this.faders.y.trigger("dragend", {
                        ...e,
                        stopPropagation: true
                    });
                if (this.getProp("spring")) {
                    this.setValue(
                        [
                            this.faders.x.getSpringValue(),
                            this.faders.y.getSpringValue()
                        ],
                        { sync: true, send: true, spring: true }
                    );
                } else {
                    this.batchDraw();
                }
    ...
    setValue(v, options = {}) {
        ...
        if (this.touched && !options.dragged && !options.doubleTap)
            return (this.setValueTouchedQueue = [v, options]);

        if (!options.dragged) {
            this.faders.x.setValue(v[0], {
                sync: false,
                send: false,
                dragged: false,
                doubleTap: options.doubleTap
            });
            this.faders.y.setValue(v[1], {
                sync: false,
                send: false,
                dragged: false,
                doubleTap: options.doubleTap
    ...
    draw() {
        ...
        var pointSize = this.pointSize,
            x = this.faders.x.percentToCoord(
                this.faders.x.valueToPercent(this.faders.x.value)
            ),
            y = this.faders.y.percentToCoord(
                this.faders.y.valueToPercent(this.faders.y.value)
            ),
        ...
        if (this.getProp("pips")) this.drawPips();
            ...
            this.clearRect = [
                [
                    x - pointSize - 2 * PXSCALE,
                    y - pointSize - 2 * PXSCALE,
                    (pointSize + 2 * PXSCALE) * 2,
                    (pointSize + 2 * PXSCALE) * 2
                ]
            ];
        }
