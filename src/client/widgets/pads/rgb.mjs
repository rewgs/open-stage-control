...
export class Rgb extends Widget {
        ...
        var defaults = super.defaults().extend({
            class_specific: {
                ...
                range: {
                    type: "object",
                    value: { min: 0, max: 255 },
                    help: "Defines the widget's output scale."
                },
                alpha: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to enable alpha channel"
                },
                rangeAlpha: {
                    type: "object",
                    value: { min: 0, max: 1 },
                    help: "Defines the widget's output scale for the alpha channel."
                }
            }
        });
    ...
    constructor(options) {
        ...
        var initValue =
            Array.isArray(this.getProp("default")) &&
            this.getProp("default").length >= 3
                ? this.getProp("default")
                : Array(3).fill(this.getProp("range").min);
        ...
        if (h != this.hsb.h || s != this.hsb.s || b != this.hsb.b) {
            ...
            for (var k in rgb) {
                rgb[k] = mapToScale(
                    rgb[k],
                    [0, 255],
                    [this.getProp("range").min, this.getProp("range").max],
                    -1
                );
            }
    ...
    dragAlphaHandle() {
        ...
        if (alpha !== this.value[3]) {
            this.setValue(
                [this.value[0], this.value[1], this.value[2], alpha],
                {
                    send: true,
                    sync: true,
                    dragged: true,
    ...
    setValue(v, options = {}) {
        ...
        if (this.touched && !options.dragged)
        ...
        for (var i = 0; i < 3; i++) {
            value[i] = clip(v[i], [
                this.getProp("range").min,
                this.getProp("range").max
            ]);
        }
        ...
        if (value.length === 4) {
            value[3] =
                v[3] === undefined
                    ? this.alpha.value
                    : clip(v[3], [
                          this.getProp("rangeAlpha").min,
                          this.getProp("rangeAlpha").max
                      ]);
        }
        ...
        this.update({
            dragged: options.dragged,
            nohue: options.nohue || (v[0] == v[1] && v[1] == v[2])
        });
    }
    ...
    update(options = {}) {
        if (!options.nohue && !options.dragged) {
            ...
            this.pad.container.setAttribute(
                "style",
                `background-color:${hueStr}`
            );
            this.hue.setValue(this.hsb.h, {
                sync: false,
                send: false,
                dragged: false
            });
...
class HueFader extends Fader {
    draw() {
        ...
        var percent = this.getProp("steps")
