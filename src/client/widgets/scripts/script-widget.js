...
class ScriptWidget extends Widget {
    ...
    static defaults() {
        ...
        defaults.scripting = {
            ...
            onKeyboard: {
                ...
                value: "",
                editor: "javascript",
                help: 'Script executed whenever the widget receives a keyboard event if `keyBinding` is set). See <a href="https://openstagecontrol.ammd.net/docs/widgets/scripting/">documentation</a>.'
            },
            ...
            keyRepeat: {
                type: "boolean",
                value: true,
                help: "Set to `false` to prevent keydown repeats when holding the key combo pressed"
            },
            keyType: {
                type: "string",
                value: "keydown",
                choices: ["keydown", "keyup", "both"],
                help: "Determines which key event trigger the script's execution"
            }
        };
        return defaults;
    }
    ...
    keyboardCallback(e) {
        ...
        if (
            this.getProp("keyType") !== "both" &&
            e.type !== this.getProp("keyType")
        )
            return;

        if (e.type === "keydown" && !this.getProp("keyRepeat"))
            e.preventRepeat();
