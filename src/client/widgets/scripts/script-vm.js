...
class ScriptVm extends Vm {
    ...
    getValueOptions() {
        return (
            this.valueOptions[this.valueOptions.length - 1] || {
                send: true,
                sync: true
            }
        );
    ...
    resolveId(id) {
        ...
        else if (id === "parent" && widget.parent !== widgetManager)
    ...
    checkContext(name) {
        if (this.widget.length === 0)
            throw name + "() cannot be called from outside the script property";
    }

    registerGlobals() {
        super.registerGlobals();

        this.sandbox.contentWindow.set = (
            id,
            value,
            extraOptions = { send: true, sync: true }
        ) => {
            ...
            if (id.includes("*")) {
                widgets = this.resolveId(
                    Object.keys(widgetManager.idRoute).filter((key) =>
                        key.match(
        ...
        this.sandbox.contentWindow.setVar = (id, name, value) => {
            ...
            for (var i = widgets.length - 1; i >= 0; i--) {
                if (widgets[i].variables[name]) {
                    ...
                    widgets[i].updateProps(
                        widgets[i].variables[name].propNames
                    );
                    widgets[i].variables[name].updated = 0;
                } else {
                    widgets[i].variables[name] = {
                        value: value,
                        propNames: []
                    };
                }
        ...
        this.sandbox.contentWindow.send = (target, address, ...args) => {
            ...
            if (target)
                overrides.target = Array.isArray(target) ? target : [target];
        ...
        this.sandbox.contentWindow.getVar = (id, name) => {
            ...
            for (var i = widgets.length - 1; i >= 0; i--) {
                if (widgets[i].variables[name])
                    return deepCopy(widgets[i].variables[name].value);
            }
        };
        ...
        this.sandbox.contentWindow.httpGet = (url, callback) => {
            ...
            if (!parser.isLocal())
                return err("httpGet error (" + url + "): non-local url");
            ...
            xhr.onerror = (e) =>
                err(`httpGet error (${url}): ${xhr.status} ${xhr.statusText}`);
            xhr.send(null);
        };
        ...
        this.sandbox.contentWindow.stateGet = (filter) => {
            ...
            if (filter) {
                ...
                var containers = filter
                    .map((x) => this.resolveId(x))
                    .reduce((a, b) => a.concat(b), []);

                if (!containers.length) return;
                filter = (widget) => {
                    return containers.some(
                        (x) => x.contains(widget) || x === widget
                    );
                };
            }
        ...
        this.sandbox.contentWindow.stateSet = (state, extraOptions = {}) => {
            ...
            stateManager.set(
                state,
                extraOptions.send === false ? false : options.send
            );
        };
        ...
        this.sandbox.contentWindow.setInterval = (id, callback, timeout) => {
            ...
            if (widget.intervals[id] !== undefined)
                clearTimeout(widget.intervals[id]);
        ...
        this.sandbox.contentWindow.toolbar = (...args) => {
            ...
            toolbar = toolbar || require("../../ui/main-menu.mjs");
        ...
        this.sandbox.contentWindow.reload = (keepState = true) => {
            ...
            if (options.onCreate) {
                ...
                widget.errorProp(
                    "onCreate",
                    "",
                    "reload() cannot be called from onCreate"
                );
