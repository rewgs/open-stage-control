...
class Clone extends Container() {
        ...
        var defaults = super.defaults().extend({
            class_specific: {
                widgetId: {
                    type: "string",
                    value: "",
                    help: "`id` of the widget to clone"
                },
                props: {
                    type: "object",
                    value: {},
                    editor: "javascript",
                    syntaxChecker: false,
                    help: "Cloned widget's properties to override"
                }
            },
            value: null,
            osc: null,
    ...
    constructor(options) {
        ...
        while (parent !== widgetManager) {
            if (parent.getProp("id") === this.getProp("widgetId")) {
                this.errorProp(
                    "widgetId",
                    "circular reference",
                    "cannot create clone: target widget is a parent of this clone."
                );
                return;
            }
        ...
        var onPropChanged = (e) => {
            ...
            if (
                e.widget === this.cloneTarget ||
                this.cloneTarget.contains(e.widget)
            ) {
        ...
        widgetManager.on("prop-changed-maybe", onPropChanged, {
            context: this
        });
    ...
    getCloneTarget() {
        ...
        var widgets = widgetManager
            .getWidgetById(this.getProp("widgetId"))
    ...
    createClone(init) {
        ...
        var data = {
            ...deepCopy(this.cloneTarget.props),
            ...this.getProp("props")
        };
    ...
    updateContainer(checkResize) {
        if (this.cloneTarget) {
            var classes = [...this.cloneTarget.container.classList].filter(
                (i) => excludedCloneClasses.indexOf(i) === -1
            );
