        ...
        return Widget.defaults().extend({
            style: {
                ...
                colorWhite: {
                    type: "string",
                    value: "auto",
                    help: "White keys color."
                },
                colorBlack: {
                    type: "string",
                    value: "auto",
                    help: "Black keys color."
                }
            },
            class_specific: {
                keys: {
                ...
                traversing: {
                    type: "boolean",
                    value: true,
                    help: "Set to `false` to disable traversing gestures"
                },
                on: {
                    type: "*",
                    value: 1,
        ...
        this.on("value-changed", (e) => {
            ...
            if (widget.getValue()) {
                if (this.getProp("velocity")) {
                    var height = widget._black
                        ? this.keyHeight * 0.65
                        : this.keyHeight;
                    value = mapToScale(
                        e.options.y,
                        [0, height * 0.9],
                        [this.getProp("off"), this.getProp("on")],
                        this.decimals
                    );
                } else {
    ...
    setValue(v, options = {}) {
        ...
        if (v.length === this.children.length) {
            for (var i = 0; i < this.children.length; i++) {
                this.children[i].setValue(
                    v[i] !== this.getProp("off") ? 1 : 0,
                    options
                );
            }
        } else if (
            v.length === 2 &&
            v[0] >= parseInt(this.getProp("start")) &&
            v[0] < 128
        ) {
            var index = v[0] - parseInt(this.getProp("start"));
            if (this.children[index]) {
                this.children[index].setValue(
                    v[1] !== this.getProp("off") ? 1 : 0,
                    options
                );
            }
        }
