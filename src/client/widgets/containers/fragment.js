...
class Fragment extends Container() {
        ...
        var defaults = super.defaults().extend({
            class_specific: {
                file: {
                    type: "string",
                    value: "",
                    help: "Fragment file path (relative to the session or theme file location by default, falling back to absolute path)"
                },
                fallback: {
                    type: "string",
                    value: "",
                    help: "Fallack fragment file path, loaded if `file` can't be opened"
                },
                props: {
                    type: "object",
                    value: {},
                    editor: "javascript",
                    syntaxChecker: false,
                    help: "Fragment widget's properties to override"
                }
            },
            value: null,
            osc: null,
    ...
    constructor(options) {
        ...
        sessionManager.on(
            ...
            (e) => {
                var { path } = e;
                if (
                    path === this.getProp("file") ||
                    path === this.getProp("fallback")
                ) {
                    this.createFragment();
                }
            },
                ...
                if (path === this.getProp("file")) {
                    if (this.getProp("fallback")) {
                        if (this.getFragment()) this.createFragment();
                            ...
                            sessionManager.loadFragment(
                                this.getProp("fallback")
                            );
                    } else {
    ...
    updateContainer(checkResize) {
        if (this.children[0]) {
            var classes = [...this.children[0].container.classList].filter(
                (i) => excludedfragmentClasses.indexOf(i) === -1
            );
