...
class PatchBayNode extends Widget {
    ...
    setValue(v, options = {}) {
        ...
        if (
            (this.parent.getProp("exclusive") === "out" ||
                this.parent.getProp("exclusive") === "both") &&
            this.value.length > 0
        ) {
            for (var input of this.parent.inputsWidgets) {
                if (
                    input !== this &&
                    input.value.some((x) => this.value.includes(x))
                ) {
                    input.setValue(
                        input.value.filter((x) => !this.value.includes(x)),
                        options
                    );
                }
            }
...
class PatchBay extends Container(Canvas) {
        ...
        var defaults = super.defaults().extend({
            class_specific: {
                ...
                outputs: {
                    ...
                    help: "List of output values the inputs can connect to (see `inputs`)."
                },
                exclusive: {
                    type: "string",
                    value: false,
                    choices: [false, "in", "out", "both"],
                    help: [
        ...
        for (let k in this.inputs) {
            let w = parser.parse({
                data: {
                    ...
                    address:
                        '#{@{parent.address} == "auto" ? "/" + @{parent.id} : @{parent.address}}',
                    target: "@{parent.target}",
                    ignoreDefaults: "@{parent.ignoreDefaults}",
                    preArgs:
                        '#{@{parent.preArgs} == "" ? @{this.comments} : Array.isArray(@{parent.preArgs}) ? @{parent.preArgs}.concat(@{this.comments}) : [@{parent.preArgs}, @{this.comments}]}',
    ...
    toggleConnection(node) {
        ...
        if (
            this.connecting[0] !== undefined &&
            this.connecting[1] !== undefined
        ) {
            this.inputsWidgets[this.connecting[0]].toggleValue(
                this.outputsValues[this.connecting[1]]
            );
            this.connecting = [];
        } else {
            this.batchDraw();
    ...
    draw() {
            ...
            if (connections.length) {
                ...
                for (j = 0; j < connections.length; j++) {
                    if (this.outputsValues.indexOf(connections[j]) > -1) {
                        y2 =
                            (this.height / this.outputsValues.length) *
                            (this.outputsValues.indexOf(connections[j]) + 0.5);

                        var line = [
                            [x1, y1],
                            [this.width / 2, y1, this.width / 2, y2, x2, y2]
                        ];

                        if (
                            this.connecting[0] === i ||
                            this.connecting[1] ===
                                this.outputsValues.indexOf(connections[j])
                        ) {
        ...
        if (this.connecting.length) {
            ...
            var side = this.connecting[0] !== undefined ? 0 : 1,
                ...
                cy =
                    (this.height /
                        (side === 0 ? this.inputsValues : this.outputsValues)
                            .length) *
                    (this.connecting[side] + 0.5);
        ...
        this.ctx.globalAlpha = strongLines.length
            ? this.cssVars.alphaFillOff
            : this.cssVars.alphaFillOn;
