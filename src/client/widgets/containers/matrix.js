...
class Matrix extends Panel {
        ...
        return super.defaults().extend({
            class_specific: {
                widgetType: {
                    type: "string",
                    value: "button",
                    help: "Defines the type of the widgets in the matrix"
                },
                quantity: {
                    type: "number",
                    value: 4,
                    help: "Defines the number of widgets in the matrix"
                },
                props: {
                    type: "object",
                    value: { label: "#{$}" },
    ...
    constructor(options) {
        ...
        if (parser.widgets[this.getProp("widgetType")]) {
            ...
            for (let i = 0; i < quantity; i++) {
                ...
                data = JSON.parse(
                    JSON.stringify(data).replace(/(JS|#|OSC|@|VAR)_\{/g, "$1{")
                );
            ...
            this.errors.widgetType =
                this.getProp("widgetType") + " is not a valid widget type";
        }
    }
        ...
        switch (propName) {
            case "quantity":
                var quantity = Math.max(
                    parseInt(this.getProp("quantity")) || 0,
                    0
                );
                ...
                if (oldPropValue > quantity) {
                    for (let i = oldPropValue - 1; i >= quantity; i--) {
                        ...
                        widgetManager.removeWidgets(
                            this.children[i]
                                .getAllChildren()
                                .concat(this.children[i])
                        );
                    }
                } else {
                    this.cachedProps.props = this.resolveProp(
                        "props",
                        undefined,
                        true
                    );
            ...
            case "props":
                for (let i = this.children.length - 1; i >= 0; i--) {
                    ...
                    if (
                        typeof oldPropValue[i] === "object" &&
                        oldPropValue[i] !== null
                    ) {
                        // if an overridden default props is removed, set it back to default
                        let overriddenDefaults, widgetDefaults;
                        for (var k in oldPropValue[i]) {
                            if (data[k] === undefined) {
                                if (
                                    Matrix.overriddenDefaults.indexOf(k) !== -1
                                ) {
                                    overriddenDefaults =
                                        overriddenDefaults ||
                                        this.defaultProps(i);
    ...
    resolveProp(
        ...
        if (propName === "props" && !this.resolveLock) {
            ...
            propValue =
                propValue !== undefined ? propValue : this.props[propName];
            ...
            if (
                ...
                for (let i = 0; i < quantity; i++) {
                    ...
                    for (var k in propValue) {
                        data[i][k] = super.resolveProp(
                            propName,
                            deepCopy(propValue[k]),
                            i === 0 ? storeLinks : false,
                            originalWidget,
                            originalPropName,
                            { $: i }
                        );
