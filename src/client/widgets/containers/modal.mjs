...
class Modal extends Panel {
        ...
        return super.defaults().extend({
            class_specific: {
                doubleTap: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to make the modal require a double tap to open instead of a single tap"
                }
            },
            geometry: {
                ...
                relative: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to make the modal's position relative to the button's position."
                },
                ignoreTabs: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to allow the modal overflowing its tab ancestors."
                }
            },
            style: {
                ...
                popupLabel: {
                    type: "string|boolean",
                    value: "auto",
                    help: "Alternative label for the modal popup"
                },
                popupPadding: {
                    type: "number",
                    value: "auto",
                    help: "Modal's inner spacing."
                },
                tabsPosition: null,
                html: null
            },
    ...
    constructor(options) {
        ...
        this.popup = html`
            <div class="popup">
                <div class="popup-wrapper">
                    <div class="popup-title closable">
                        <span class="popup-label"></span
                        ><span class="closer">${raw(icon("times"))}</span>
                    </div>
                    <div
                        class="popup-content widget panel-container not-editable contains-widgets"
                    ></div>
                </div>
            </div>
        `;
        ...
        for (var g of ["Width", "Height", "Left", "Top"]) {
            geometry[g] =
                parseFloat(this.getProp("popup" + g)) ==
                this.getProp("popup" + g)
                    ? parseFloat(this.getProp("popup" + g)) + "rem"
                    : this.getProp("popup" + g);
        }
        ...
        this.toggle = this.container.appendChild(
        ...
        this.on(
            ...
            (e) => {
                if (
                    (e.target === this.popup || e.target === closer) &&
                    this.value == 1
                ) {
    ...
    fixParents() {
        ...
        while (
            parent &&
            parent.props &&
            !String(parent.getProp("type")).match(stop)
        ) {
            parent.modalBreakout += this.value ? 1 : -1;
            if (parent.modalBreakout > 0)
                parent.container.classList.add("modal-breakout");
            else if (parent.modalBreakout === 0)
                parent.container.classList.remove("modal-breakout");
        ...
        if (this.modalBreakout > 0)
            this.container.classList.add("modal-breakout");
        else if (this.modalBreakout === 0)
            this.container.classList.remove("modal-breakout");
    ...
    updateLabel() {
        ...
        if (this.getProp("label") === false) {
            if (this.container.contains(this.label))
    ...
    updatePopupLabel() {
        ...
        var label =
            this.getProp("popupLabel") !== "auto"
                ? iconify(this.getProp("popupLabel"))
                : this.label.innerHTML;

        DOM.get(this.popup, ".popup-title .popup-label")[0].innerHTML = label;
        this.popup.classList.toggle(
            "no-label",
            this.getProp("popupLabel") === false
        );
    ...
    setScroll(x, y) {
        if (x !== undefined)
            this.popupContent.firstChild.scrollLeft = this.scroll[0] = x;
        if (y !== undefined)
            this.popupContent.firstChild.scrollTop = this.scroll[1] = y;
    }
}
