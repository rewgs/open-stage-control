...
export class Plot extends StaticProperties(Canvas, { bypass: true }) {
    static description() {
        ...
        return super.defaults().extend({
            ...
            style: {
                ...
                dots: {
                    type: "boolean",
                    value: false,
                    help: "Draw dots on the line"
                },
                bars: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to use draw bars instead (disables `logScaleX` and forces `x axis` even spacing)"
                },
                pips: {
                    type: "boolean",
                    value: true,
            ...
            class_specific: {
                rangeX: {
                    type: "object",
                    value: { min: 0, max: 1 },
                    help: "Defines the min and max values for the x axis"
                },
                rangeY: {
                    type: "object",
                    value: { min: 0, max: 1 },
    ...
    constructor(options) {
        ...
        this.pips = {
            x: {
                min:
                    Math.abs(this.rangeX.min) >= 1000
                        ? this.rangeX.min / 1000 + "k"
                        : this.rangeX.min,
                max:
                    Math.abs(this.rangeX.max) >= 1000
                        ? this.rangeX.max / 1000 + "k"
                        : this.rangeX.max
            },
            y: {
                min:
                    Math.abs(this.rangeY.min) >= 1000
                        ? this.rangeY.min / 1000 + "k"
                        : this.rangeY.min,
                max:
                    Math.abs(this.rangeY.max) >= 1000
                        ? this.rangeY.max / 1000 + "k"
                        : this.rangeY.max
            }
        };
    }
    ...
    draw_line() {
        ...
        for (i = 0; i < length; i++) {
            if (this.value[i].length) {
                points.push(
                    mapToScale(
                        this.value[i][0],
                        [this.rangeX.min, this.rangeX.max],
                        [padding, this.width - padding],
                        -1,
                        this.logScaleX,
                        true
                    )
                ...
                points.push(
                    mapToScale(
                        this.value[i][1],
                        [this.rangeY.min, this.rangeY.max],
                        [
                            this.height - 2 * PXSCALE - padding,
                            2 * PXSCALE + padding
                        ],
                        -1,
                        this.logScaleY,
                ...
                if (
                    i < length - 2 &&
                    this.value[i] === previousValue &&
                    this.value[i + 1] === previousValue
                ) {
                    continue;
                }
                nx = mapToScale(
                    i,
                    [0, this.value.length - 1],
                    [padding, this.width - padding],
                    -1,
                    this.logScaleX,
                    true
        ...
        if (this.getProp("origin") !== false) {
            var origin = mapToScale(
                this.getProp("origin"),
                [this.rangeY.min, this.rangeY.max],
                [this.height - padding, padding],
                0,
                this.getProp("logScaleY"),
                true
            );
    ...
    draw_bars() {
        var padding = this.cssVars.padding,
            barWidth = Math.round(
                (this.width - padding * 2) / this.value.length
            ),
            offset = Math.round(
                (this.width - barWidth * this.value.length) / 2
            );

        var origin = mapToScale(
            this.getProp("origin") !== false
                ? this.getProp("origin")
                : this.rangeY.min,
            [this.rangeY.min, this.rangeY.max],
            [this.height - padding, padding],
        ...
        for (let i in this.value) {
            var y = mapToScale(
                this.value[i].length ? this.value[i][1] : this.value[i],
                [this.rangeY.min, this.rangeY.max],
                [this.height - 2 * PXSCALE - padding, 2 * PXSCALE + padding],
                0,
                this.logScaleY,
                true
            );
