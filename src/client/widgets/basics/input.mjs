...
class Input extends Canvas {
        ...
        return super.defaults().extend({
            style: {
                ...
                unit: {
                    type: "string",
                    value: "",
                    help: "Unit will be appended to the displayed widget's value (it doesn't affect osc messages)"
                }
            },
            class_specific: {
                asYouType: {
                    type: "boolean",
                    value: false,
                    help: "Set to `true` to make the input send its value at each keystroke"
                },
                numeric: {
                    type: "boolean|number",
                    value: false,
    ...
    constructor(options) {
        ...
        if (this.getProp("align") === "right")
            this.widget.classList.add("right");
        ...
        if (this.getProp("interaction")) {
            ...
            if (this.getProp("validation") !== "") {
                var validation = String(this.getProp("validation")),
                    flags = validation.match(/^\/.*\/.*$/)
                        ? validation.split("/").pop()
                        : "gm",
                    regExpString = validation.match(/^\/.*\/.*$/)
                        ? validation.replace(/^\/(.*)\/.*$/, "$1")
                        : validation;
    ...
    setValue(v, options = {}) {
        ...
        if (
            this.maxLength !== -1 &&
            this.getStringValue(v).length > this.maxLength
        ) {
...
Input.dynamicProps = Input.prototype.constructor.dynamicProps.filter(
    (x) => x !== "interaction"
);
