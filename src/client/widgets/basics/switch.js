...
class Switch extends MenuBase {
        ...
        return super.defaults(Switch).extend({
            style: {
                ...
                layout: {
                    ...
                    help: "Defines how items should be laid out"
                },
                gridTemplate: {
                    type: "string|number",
                    value: "",
    ...
    constructor(options) {
        ...
        if (this.getProp("wrap") === "soft")
            this.container.classList.add("wrap-soft");
        ...
        if (
            this.getProp("mode") === "tap" ||
            this.getProp("mode") === "slide"
        ) {
            this.on("draginit", dragCallback, { element: this.widget });
            this.on(
                "drag",
                (e) => {
                    if (this.getProp("mode") === "slide" || e.traversing)
                        dragCallback(e, true);
                },
                { element: this.widget }
            );
            ...
            this.on(
                ...
                () => {
                    var i =
                        (this.getIndex(this.value) + 1) % this.values.length;
