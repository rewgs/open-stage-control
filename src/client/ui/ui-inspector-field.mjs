...
class UiInspectorField extends UiWidget {
    constructor(options) {
        ...
        this.container = html`<osc-inspector-field
            name="${this.name}"
        ></osc-inspector-field>`;
        ...
        if (this.name === "type" || this.name === "widgetType")
    ...
    createTypeSelect() {
        if (
            this.widget.props.type == "tab" ||
            this.widget.props.type == "root"
        ) {
...
${this.value}</textarea
    ...
    createInput() {
        ...
        let input = html`<textarea
            ...
            class="no-keybinding"
            name="${this.name}"
            rows="${stringValue.split("\n").length}"
        ></textarea>`;

        if (this.name === "id" && this.widget.props.type === "root")
            input.disabled = true;
        ...
        if (this.default.type.includes("boolean")) {
            let toggle = html`
                <osc-inspector-checkbox
                    name="${this.name}"
                    class="${this.widget.getProp(this.name) ? "on" : ""}"
                >
                    ${raw(icon("check"))}
                </osc-inspector-checkbox>
            `;
        ...
        if (this.name.includes("color")) {
            let style = window.getComputedStyle(this.widget.container),
                colorData = this.widget.constructor.cssVariables.find(
                    (x) => x.js === this.name
                ),
                val = "transparent";

            try {
                val = chroma(style.getPropertyValue(colorData.css).trim()).hex(
                    "rgba"
                );
            } catch (e) {}
            this.container.appendChild(html`
                <osc-inspector-color
                    ...
                    name="${this.name}"
                    value="${val}"
                    style="--color-picker-value: ${val}"
                ></osc-inspector-color>
            `);
        } else if (
            stringValue === "auto" &&
            this.widget.constructor.cssVariables.some((x) => x.js === this.name)
        ) {
            let style = window.getComputedStyle(this.widget.container),
                data = this.widget.constructor.cssVariables.find(
                    (x) => x.js === this.name
                ),
