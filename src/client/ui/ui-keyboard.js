...
class OscKeybard extends UiWidget {
    constructor(options) {
        ...
        this.container.appendChild(
            html`<div class="row">${this.display}</div>`
        );

        for (var k in layout) {
            layout[k] = layout[k].map((r, i) =>
                (r + " " + (numericLayout[i] || "")).trim().split(" ")
            );
        }
        ...
        for (var i = 0; i < layout.default.length; i++) {
            ...
            for (var j = 0; j < rowData.length; j++) {
                var keyData = rowData[j],
                    ...
                    key = html` <key>
                        <span class="display-default"
                            >${raw(keysDisplay[keyData] || keyData)}</span
                        >
                        <span class="display-shift"
                            >${raw(keysDisplay[shiftData] || shiftData)}</span
                        >
                    </key>`;
        ...
        this.on(
            ...
            (e) => {
                ...
                if (e.target.tagName === "KEY") {
                    ...
                    this.keyDown(
                        key.getAttribute(
                            this.shift !== this.lock ? "data-shift" : "data-key"
                        )
                    );
    ...
    keyDown(key) {
        ...
        switch (key) {
            ...
            case "{bksp}":
                ...
                this.input.value =
                    this.input.value.substring(0, startPos - 1) +
            ...
            case "{space}":
                ...
                break;
            case "{left}":
                this.input.selectionStart = this.input.selectionEnd = Math.max(
                ...
                if (this.input.value.includes("\n")) {
                    ...
                    for (let i = 0; i < chars.length; i++) {
                        ...
                        if (charCount > this.input.selectionStart) {
                            posAtLine =
                                chars[i] -
                                (charCount - this.input.selectionStart);
                            break;
                        }
                        line++;
                    }
                    ...
                    if (posAtLine > chars[line] - 1)
                        posAtLine = chars[line] - 1;

                    this.input.selectionStart = this.input.selectionEnd =
                        newPos + posAtLine;
                } else {
                    this.input.selectionStart = this.input.selectionEnd =
                        key === "{up}" ? 0 : this.input.value.length;
                }
                this.keydownHandler();
                return;
            default:
    ...
    updateCursor() {
        ...
        var caretChar = DOM.get(
            this.display,
            "char:nth-child(" + start + ")"
        )[0];
