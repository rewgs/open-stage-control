...
class UiTree extends UiWidget {
    constructor(options) {
        ...
        this.mounted = false;
        this.fragment = this.container.appendChild(
            html`<div
                class="fragment-mode-warning"
                title="${locales("fragment_mode_explanation")}"
            >
                ${locales("fragment_mode_warning")}
            </div>`
        );
        this.filter = this.container.appendChild(
            html`<input
                class="filter"
                type="text"
                placeholder="${locales("tree_filter")}..."
            />`
        );
        ...
        this.sortCallback = (event) => {
            var from =
                    widgetManager.widgets[event.from.getAttribute("data-hash")],
                to = widgetManager.widgets[event.to.getAttribute("data-hash")];

            if (!to || (from === to && event.oldIndex === event.newIndex))
                return;
            ...
            this.sortables.push(
                new Sortable(el, {
                    group: {
                        name: "group",
                        pull: (to, from) =>
                            from.el.dataset.childrenType ===
                                to.el.dataset.childrenType ||
                            to.el.dataset.childrenType === "",
                        put: (to, from) =>
                            from.el.dataset.childrenType ===
                                to.el.dataset.childrenType ||
                    ...
                    onStart: (event) => {
                        placeholder = event.from.insertBefore(
                            event.clone.cloneNode(true),
    ...
    showWidgetInSession(widget) {
        ...
        while (parent !== widgetManager) {
            if (parent.getProp("type") === "tab")
                parent.parent.setValue(parent.parent.children.indexOf(parent), {
                    sync: true,
                    send: true
                });
            else if (parent.getProp("type") === "modal")
                parent.setValue(1, { sync: true, send: true });
    ...
    parseWidgets(widget, selectedWidgets, depth = 1) {
        ...
        var selected = selectedWidgets.includes(widget),
            ...
            node = html`<li
                class="${selected ? "editing" : ""} ${!widget.getProp("visible")
                    ? "invisible"
                    : ""}"
                data-widget="${widget.hash}"
                data-type="${widget.getProp("type")}"
            >
                ${raw(
                    icon(widgetIcons[widget.getProp("type")] || "root")
                )}${id}${widget.getProp("lock") ? raw(icon("lock")) : ""}
            </li>`;
        ...
        if (
            ...
            !(widget instanceof Matrix || widget instanceof Keyboard)
        ) {
            node.insertBefore(
                html`<span class="toggle no-widget-select"></span>`,
                node.childNodes[0]
            );
            ...
            var sublist = node.appendChild(
                html`<ol
                    data-hash="${widget.hash}"
                    data-children-type="${widget.childrenType}"
                    style="--depth:${++depth};"
                ></ol>`
            );
            for (let child of widget.children) {
                if (child)
                    sublist.appendChild(
                        this.parseWidgets(child, selectedWidgets, depth)
                    );
            }
        }
    ...
    applyFilter() {
        ...
        if (!filter && !types) {
            ...
            DOM.each(this.list, "li", (element) => {
                ...
                if (types && !types.includes(element.dataset.type))
                    match = false;
            ...
            DOM.each(this.list, ".container.filter-hide", (element) => {
                if (show.some((el) => element.contains(el)))
