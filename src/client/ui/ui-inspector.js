...
class UiInspector extends UiWidget {
    constructor(options) {
        ...
        this.toolbar = this.container.appendChild(html`
            <div class="toolbar">
                <div class="btn toolbar-btn" data-action="zoom-out">
                    ${raw(icon("magnifying-glass-minus"))}
                </div>
                <div class="btn toolbar-btn" data-action="zoom-reset">
                    ${raw(icon("magnifying-glass"))}
                </div>
                <div class="btn toolbar-btn" data-action="zoom-in">
                    ${raw(icon("magnifying-glass-plus"))}
                </div>
                <div class="btn toolbar-btn" data-action="selected-id">
                    <span></span>
                </div>
                <div class="btn toolbar-btn" data-action="reset-selection">
                    ${raw(icon("border-none"))}
                </div>
                <div class="btn toolbar-btn" data-action="toggle-grid">
                    ${raw(icon("table-cells"))}
                </div>
            </div>
        `);
        this.toolbarBtns = {};
        DOM.each(this.toolbar, ".btn", (item) => {
            ...
            if (action) {
                if (action !== "selected-id")
                    item.setAttribute(
                        "title",
                        locales("inspector_" + action.replace("-", "_"))
                    );
                this.toolbarBtns[action] = item;
            }
        });
        zoom.on("local-zoom-changed", () => {
            this.toolbarBtns["zoom-out"].classList.toggle(
                "active",
                zoom.localZoom < 1
            );
            this.toolbarBtns["zoom-in"].classList.toggle(
                "active",
                zoom.localZoom > 1
            );
        });

        this.content = this.container.appendChild(
        ...
        this.container.addEventListener("fast-click", (e) => {
            if (e.target.classList.contains("category-header")) {
                ...
                if (e.detail.altKey) {
                    DOM.each(
                        this.container,
                        "osc-inspector-category.expanded",
                        (item) => {
                            let iname =
                                    item.firstChild.getAttribute("data-name"),
        ...
        this.colorPicker.on("change", (event) => {
            var textarea = DOM.get(
                this.container,
                `textarea[name="${this.colorPicker.name}"]`
            );
            if (textarea) {
    ...
    inspect(widgets) {
        ...
        for (let categoryName in props) {
            ...
            let category = html`<osc-inspector-category
                    class="${this.expandedCategories.indexOf(categoryName) > -1
                        ? "expanded"
                        : ""}"
                ></osc-inspector-category>`,
                categoryLabel =
                    categoryName === "class_specific"
                        ? widget.props.type
                        : categoryName;

            if (widgets.length > 1 && categoryName === "class_specific")
                categoryLabel = "shared properties";
            ...
            category.appendChild(html`
                ...
                >
                    ${categoryLabel}${categoryName === "widget" &&
                    widgets.length > 1
                        ? `s (${widgets.length})`
                        : ""}
                </div>
            ...
            for (let propName in props[categoryName]) {
                ...
                if (
                    widgets.some((w) => {
                        ...
                        for (let c in defaults[widget.props.type]) {
                            if (
                                defaults[w.props.type][c] &&
                                defaults[w.props.type][c].hasOwnProperty(
                                    propName
                                )
                            ) {
                                shared = true;
                ...
                if (propName.includes("_separator")) {
                    ...
                    field = html`<div class="separator">
                        ${props[categoryName][propName]}
                    </div>`;
                } else {
                    var def =
                        defaults[widget.props.type][categoryName][propName];
        ...
        this.toolbarBtns["selected-id"].firstChild.innerHTML = widgets
            .map((x) => x.getProp("id"))
            .join(", ");
        ...
        if (this.colorPicker.opened) {
            var picker = DOM.get(
                this.container,
                `osc-inspector-color[name="${this.colorPicker.name}"]`
            );
            if (picker && picker[0]) {
    ...
    onChange(event) {
        ...
        if (document.activeElement === input._ace_input)
            input._ace_input.blur();
    ...
    onKeydown(event) {
        if (event.target.tagName === "TEXTAREA") {
            ...
            event.target.classList.contains("category-header")
        ) {
            event.preventDefault();
    ...
    onInput(event) {
        ...
        this.focusedInput.setAttribute(
            "rows",
            this.focusedInput.value.split("\n").length
        );
    }

    help(name) {
        var _defaults = defaults[this.widget.getProp("type")],
            category = Object.keys(_defaults).find((x) =>
                _defaults[x].hasOwnProperty(name)
            ),
            defaultValue = _defaults[category][name],
            dynamic = this.widget.isDynamicProp(name),
            htmlHelp = Array.isArray(defaultValue.help)
                ? defaultValue.help.join("<br/><br/>").replace(/<br\/>-/g, "-")
                : defaultValue.help;

        htmlHelp = htmlHelp
            ? html`<p class="help">
                  ${raw(htmlHelp.replace(/`([^`]*)`/g, "<code>$1</code>"))}
              </p>`
            : "";
        ...
        var modal = new UiModal({
            closable: true,
            title: html`<span class="editor-help-title">${name}</span>`,
            html: true,
            content: html`
                <div class="inspector-help">
                    <div class="header">
                        ...
                        <p>
                            Default:
                            <code>${JSON.stringify(defaultValue.value)}</code>
                        </p>
                        ${defaultValue.choices
                            ...
                            : ""}
                        <p>
                            Dynamic: <code>${dynamic ? "true" : "false"}</code>
                        </p>
                    </div>
                    <div class="description">${htmlHelp}</div>
                    <div class="computed">
                        <p>
                            Computed property:
                            <code class="pre">${computedValue || "empty"}</code>
                        </p>
                    </div>
                    ${name === "value"
                        ? raw(`
