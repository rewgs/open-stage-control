    ...
    constructor(options) {
        ...
        this.container = html`
            <div id="drag-resize">
                ...
                <div
                    class="no-widget-select handle nw"
                    data-action="left,top"
                ></div>
                <div
                    class="no-widget-select handle e"
                    data-action="width"
                ></div>
                <div
                    class="no-widget-select handle s"
                    data-action="height"
                ></div>
                <div
                    class="no-widget-select handle se"
                    data-action="width,height"
                ></div>
            </div>
        `;
        ...
        this.on(
            ...
            (event) => {
                ...
                this.trigger(
                    this.dragging.indexOf("left") > -1 ? "move" : "drag-resize",
                    this
                );
                this.dragging = null;
                this.copying = null;
    ...
    updateRectangle() {
        ...
        fastdom.measure(() => {
            var lefts = widgets.map((w) => w.container.offsetLeft),
                ...
                rights = widgets.map(
                    (w) => w.container.offsetLeft + w.container.offsetWidth
                ),
                bottoms = widgets.map(
                    (w) => w.container.offsetTop + w.container.offsetHeight
                );
            ...
            fastdom.mutate(() => {
                for (var i in handlesVisibility) {
                    this.handles[i].style.display = handlesVisibility[i]
                        ? ""
                        : "none";
                }
