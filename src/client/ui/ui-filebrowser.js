...
module.exports = function UiFilebrowser(options, callback) {
    ...
    var container = DOM.get(popup.container, ".popup-content")[0],
        ...
        saveInput = html`<input
            type="text"
            class="save-as no-keybinding"
            placeholder="${options.extension ? "*." + options.extension : ""}"
            ${options.save ? "" : "disabled"}
        />`,
        actions = html`
            <div class="file-actions">
                ...
                ${save
                    ? html`<div class="btn submit save">
                          ${raw(icon("save"))} Save
                      </div>`
                    : html`<div class="btn submit open">
                          ${raw(icon("folder-open"))} Open
                      </div>`}
            </div>
        `;
    ...
    var extRe = options.extension
        ? new RegExp(".*\\." + options.extension + "$")
        : /.*/;

    if (save) {
        saveInput.addEventListener("change", () => {
            if (!saveInput.value.match(extRe))
                saveInput.value += "." + options.extension;
        });
    ...
    function submit() {
        ...
        if (
            choice &&
            choice.classList.contains("folder") &&
            (!save || !saveInputFocus)
        ) {
    ...
    function keyDownHandler(e) {
        if (e.key == "Enter") {
            ...
            for (let i in files) {
                if (
                    files[i].name[0].toLowerCase() === letter &&
                    !list.childNodes[i].firstElementChild.checked
                ) {
                    if (index === -1) index = i;
            ...
            for (let i in files) {
                ...
                list.appendChild(html`
                    <div class="file">
                        ...
                        <div class="label">
                            <span
                                >${raw(icon(file.folder ? "folder" : "osc"))}
                                ${file.name}</span
                            >
                        </div>
                    </div>
    ...
    ipc.send("listDir", {
        ...
        path: options.directory ? [options.directory] : null
    });
