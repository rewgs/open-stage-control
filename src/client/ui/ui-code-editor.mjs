...
class CodeEditor {
    constructor(parent, name, language, syntaxChecker) {
        ...
        this.helpBtn = html`<div class="btn help">
            ${locales("editor_ace_help")}
        </div>`;
        this.fsBtn = html`<div class="btn fullscreen">
            ${raw(icon("expand"))}
        </div>`;
    ...
    bind(widget, field) {
        ...
        if (this.language === "javascript") {
            ...
            if (this.name === "onKeyboard") {
                ...
                globals =
                    this.widget.getProp("type") === "canvas"
                        ? {
                              value: true,
                              height: true,
                              width: true,
    ...
    fullscreen() {
        ...
        this.field.style.setProperty(
            "--prefix",
            this.editor.fullscreen ? '"' + this.widget.getProp("id") + '."' : ""
        );
        setTimeout(() => {
    ...
    help() {
        ...
        var modal = new UiModal({
            closable: true,
            title: html`<span class="editor-help-title"
                >${locales("editor_ace_help_title")}</span
            >`,
            html: true,
            content: html`
                <div class="inspector-help">
                    <table>
                        ...
                        ${keybindings.map(
                            (k) =>
                                html`<tr>
                                    <td><span class="kbd">${k.key}</span></td>
                                    <td>${k.command}</td>
                                </tr>`
                        )}
                    </table>
