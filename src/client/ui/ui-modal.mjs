...
class UiModal extends UiWidget {
    constructor(options) {
        ...
        if (!MODAL_CONTAINER)
            MODAL_CONTAINER = DOM.get("osc-modal-container")[0];
        ...
        this.container = html`
            <div class="popup show">
                <div class="popup-wrapper">
                    <div
                        class="popup-title ${options.closable
                            ? "closable"
                            : ""}"
                    >
                        <span class="title">
                        ...
                        ${options.closable
                            ? html`<span class="closer"
                                  >${raw(icon("times"))}</span
                              >`
                            : ""}
                    </div>
                    <div
                        class="popup-content ${options.html
                            ? "html"
                            : "message"}"
                    >
                        ${options.content}
        ...
        if (options.width)
            this.container.style.setProperty("--width", options.width + "rem");
        if (options.height)
