...
function updateWidget(widget, options = {}) {
    if (options.changedProps) {
        ...
        if (
            ...
            var edited =
                widget.updateProps(options.changedProps, null, {
                    fromEditor: true
                }) || widget;
            ...
            if (
                editor.selectedWidgets.includes(widget) &&
                !options.preventSelect
            ) {
    ...
    if (!widget.mounted) {
        console.debug(
            `updateWidget prevented on ${widget.getProp("id")} (widget not mounted yet)`
        );
        return;
    }
    ...
    if (reuseChildren) {
        ...
        if (options.removedIndexes) {
            options.removedIndexes = options.removedIndexes.sort(
                (a, b) => b - a
            );
    ...
    var removedWidgets = reuseChildren
        ? removedChildren
              .map((x) => x.getAllChildren().concat(x))
              .concat(widget)
    ...
    for (let w of removedWidgets) {
        if (widgetManager.widgets[w.hash]) {
            ...
            if (valueProp !== "" && valueProp !== undefined)
                stateManager.pushValueOldProp(id, valueProp);
        }
    ...
    newWidget.container.parentNode.replaceChild(
        newWidget.container,
        widget.container
    );

    if (newWidget.getProp("type") === "tab")
        newWidget.parent.trigger("tab-created", { widget: widget });
    ...
    if (stateManager.queueCounter === 0) {
        for (let id in scrollState) {
            for (let w of widgetManager.getWidgetById(id)) {
                if (w.getProp("scroll"))
                    w.setScroll(scrollState[id][0], scrollState[id][1]);
            }
        }
    }
...
var incrementWidget = function (data, root) {
    ...
    if (id && address == "/" + id) {
        ...
        !address.match(
            /^(auto|\/control|\/note|\/program|\/sysex|\/mtc|\/key_pressure|\/channel_pressure|\/pitch)$/
        )
    ) {
        var addressref = widgetManager.createAddressRef(
            null,
            data.preArgs,
            address
        );
        ...
        while (
            fakeStore.address.indexOf(address) != -1 ||
            widgetManager.getWidgetByAddress(addressref).length
        ) {
    ...
    if (id) {
        while (
            fakeStore.id.indexOf(id) != -1 ||
            widgetManager.getWidgetById(id).length
        ) {
