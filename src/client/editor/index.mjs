// var { updateWidget, incrementWidget } = require("./data-workers.mjs"),
//     keyboardJS = require("keyboardjs/dist/keyboard.min.js"),
//     { diff, diffToWidget } = require("./diff.mjs"),
...
class Editor {
    constructor() {
        ...
        this.inspector.on("update", (event) => {
            ...
            if (
                this.selectedWidgets.some((w) => w.getProp("lock")) &&
                propName !== "lock"
            ) {
            ...
            for (var w of this.selectedWidgets) {
                ...
                if (
                    (propName === "label" || propName === "popupLabel") &&
                    value === true
                ) {
                    ...
                    w.props[propName] =
                        value !== ""
                            ? value
                            : deepCopy(
                                  defaults[w.props.type]._props()[propName]
                              );
                }
                ...
                try {
                    newWidgets.push(
                        updateWidget(w, {
                            ...
                            preventSelect: this.selectedWidgets.length > 1
                        })
                    );
                } catch (e) {
        ...
        this.widgetTree = new UiTree({
            selector: "#osc-tree",
            parent: leftUiSidePanel
        });
        this.widgetTree.on("sorted", (event) => {
            ...
            if (from === to) {
                ...
                to.props[propName].splice(
                    newIndex,
                    0,
                    to.props[propName].splice(oldIndex, 1)[0]
                );

                var indices = [newIndex, oldIndex];
                if (Math.abs(oldIndex - newIndex) > 1) {
                    for (
                        var i = Math.min(newIndex, oldIndex) + 1;
                        i < Math.max(newIndex, oldIndex);
                        i++
                    ) {
                        indices.push(i);
                    }
                ...
                container = updateWidget(to, {
                    removedIndexes: indices,
                    addedIndexes: indices,
                    preventSelect: true
                });
                ...
                to.props[propName].splice(
                    newIndex,
                    0,
                    from.props[propName][oldIndex]
                );
                from.props[propName].splice(oldIndex, 1);

                updateWidget(from, {
                    removedIndexes: [oldIndex],
                    preventSelect: true
                });
                container = updateWidget(to, {
        ...
        this.selectarea = new UiSelectArea(
            "[data-widget]:not(.not-editable)",
            (elements, e) => {
                var widgets = elements
                    .map((e) =>
                        widgetManager.getWidgetByElement(
                            e,
                            ":not(.not-editable)"
                        )
                    )
                ...
                if (e.ctrlKey) {
                    ...
                    for (var i in widgets) {
                        this.select(widgets[i], {
                            multi: true,
                            fromLasso: true,
                            lassoEnd: i == widgets.length - 1
                        });
                    }
                } else {
                    ...
                    if (
                        ...
                        groups = groups.filter(
                            (g) =>
                                g[0].parent === this.selectedWidgets[0] ||
                                g[0].parent === this.selectedWidgets[0].parent
                        );
                    }

                    var lengths = groups.map((g) => g.length),
                        selection =
                            groups[lengths.lastIndexOf(Math.max(...lengths))];
    ...
    handleKeydown(combo, e) {
        ...
        switch (combo) {
            ...
            case "alt + left":
                if (
                    !this.selectedWidgets.length ||
                    this.selectedWidgets[0].parent === widgetManager
                )
                    return;

                if (
                    this.selectedWidgets[0].props.height === undefined &&
                    e.key.match(/Arrow(Up|Down)/)
                )
                    return;
                if (
                    this.selectedWidgets[0].props.width === undefined &&
                    e.key.match(/Arrow(Left|Right)/)
                )
                    return;
            ...
            case "mod + left":
                ...
                if (
                    this.selectedWidgets[0].props.top === undefined &&
                    e.key.match(/Arrow(Up|Down)/)
                )
                    return;
                if (
                    this.selectedWidgets[0].props.left === undefined &&
                    e.key.match(/Arrow(Left|Right)/)
                )
                    return;
            ...
            case "left":
                ...
                if (
                    e.key === "ArrowLeft" &&
                    curWidget.parent !== widgetManager
                ) {
                    toSelect = curWidget.parent;
                } else if (e.key === "ArrowRight") {
                    if (
                        curWidget.children[0] &&
                        !curWidget.children[0]._not_editable
                    ) {
                    ...
                    if (toSelectList && toSelectList.length) {
                        toSelectList.sort(
                            (a, b) =>
                                a.container.offsetLeft > b.container.offsetLeft
                        );
            ...
            case "end":
                if (
                    ...
                    this.selectedWidgets[0].parent.children.length <= 1
                )
                    return;
            ...
            case "f2":
                if (!this.selectedWidgets.length) return;
                if (
                    this.selectedWidgets[0].props.label === undefined &&
                    this.selectedWidgets[0].props.html === undefined
                )
                    return;
                if (!this.inspector.expandedCategories.includes("style")) {
                    var header = DOM.get(
                        this.inspector.container,
                        '.category-header[data-name="style"]'
                    )[0];
                    if (header) {
            ...
            case "mod + f":
                if (this.widgetTree.parent.minimized)
                    this.widgetTree.parent.restore();
    ...
    handleKeyup(combo, e) {
        if (
            ...
            (e &&
                e.target &&
                e.target.tagName &&
                e.target.tagName.match(/INPUT|TEXTAREA|SELECT/))
        )
            return;
    ...
    enable() {
        ...
        this.oscContainer.addEventListener(
            "mouseleave",
            this.mouveLeaveHandler
        );
    ...
    disable() {
        ...
        this.oscContainer.removeEventListener(
            "mousemove",
            this.mouveMoveHandler
        );
        this.oscContainer.removeEventListener(
            "mouseleave",
            this.mouveLeaveHandler
    ...
    select(widget, options = {}) {
        ...
        if (Array.isArray(widget)) {
            ...
            while (!sameLevel && widget.parent !== widgetManager) {
                ...
                if (
                    this.selectedWidgets.length &&
                    this.selectedWidgets[0].parent !== widget.parent
                )
                    test = false;
                sameLevel = test;
            ...
            if (!this.selectedWidgets.includes(widget) && sameLevel) {
                ...
                this.selectedWidgets.splice(
                    this.selectedWidgets.indexOf(widget),
                    1
                );
            }
        } else {
    ...
    duplicateWidget(x, y, increment) {
        ...
        var index = Math.max(
            ...this.selectedWidgets.map((w) => w.parent.children.indexOf(w))
        );
    ...
    pasteWidget(x, y, increment, index, tmp) {
        ...
        if (
            this.selectedWidgets[0].childrenType === undefined ||
            (clipboard[0].type === "tab" &&
                this.selectedWidgets[0].childrenType === "widget") ||
            (clipboard[0].type !== "tab" &&
                this.selectedWidgets[0].childrenType === "tab")
        )
            return;
        ...
        if (x !== undefined) {
            for (let i in pastedData) {
                ...
                if (typeof top === "string" && top.includes("%")) {
                    top =
                        (parseFloat(top) / 100) *
                        this.selectedWidgets[0].widget.offsetHeight;
                ...
                if (typeof left === "string" && left.includes("%")) {
                    left =
                        (parseFloat(left) / 100) *
                        this.selectedWidgets[0].widget.offsetWidth;
            ...
            for (let i in pastedData) {
                if (!String(pastedData[i].left).match(/\{/)) {
                    if (pastedData[i]._pleft) {
                        pastedData[i].left =
                            (
                                ((pastedData[i]._aleft - minLeft + x) * 100) /
                                this.selectedWidgets[0].widget.offsetWidth
                            )
                                .toFixed(2)
                                .replace(/\.?0+$/, "") + "%";
                ...
                if (!String(pastedData[i].top).match(/\{/)) {
                    if (pastedData[i]._ptop) {
                        pastedData[i].top =
                            (
                                ((pastedData[i]._atop - minTop + y) * 100) /
                                this.selectedWidgets[0].widget.offsetHeight
                            )
                                .toFixed(2)
                                .replace(/\.?0+$/, "") + "%";
        ...
        this.selectedWidgets[0].props[store] =
            this.selectedWidgets[0].props[store] || [];
    ...
    pasteWidgetAsClone(x, y) {
        ...
        if (
            !this.idClipboard ||
            !widgetManager.getWidgetById(this.idClipboard).length
        )
            return;
        ...
        if (x !== undefined) {
            ...
            if (this.usePercents) {
                clone.top =
                    ((100 * y) / editor.selectedWidgets[0].widget.offsetHeight)
                        .toFixed(2)
                        .replace(/\.?0+$/, "") + "%";
                clone.left =
                    ((100 * x) / editor.selectedWidgets[0].widget.offsetWidth)
                        .toFixed(2)
                        .replace(/\.?0+$/, "") + "%";
            }
        }

        this.selectedWidgets[0].props.widgets =
            this.selectedWidgets[0].props.widgets || [];
        this.selectedWidgets[0].props.widgets.push(clone);

        var indexes = {
            addedIndexes: [this.selectedWidgets[0].props.widgets.length - 1]
        };
    ...
    deleteWidget() {
        ...
        var type =
                this.selectedWidgets[0].props.type == "tab" ? "tab" : "widget",
            parent = this.selectedWidgets[0].parent,
            index = this.selectedWidgets
                .map((w) => parent.children.indexOf(w))
                .sort((a, b) => {
                    return b - a;
                }),
            removedIndexes = [];
        ...
        var container = updateWidget(parent, {
            preventSelect: true,
            removedIndexes
        });
        if (container.children.length) {
            this.select(
                container.children[
                    Math.min(index.pop(), container.children.length - 1)
                ]
            );
        } else {
    ...
    resizeWidget(deltaW, deltaH) {
        ...
        for (var i = 0; i < this.selectedWidgets.length; i++) {
            ...
            nW =
                ...
                (deltaW * w.container.offsetWidth) / area.initWidth;
            nH =
                w.container.offsetHeight +
                (deltaH * w.container.offsetHeight) / area.initHeight;

            if (this.selectedWidgets.length > 1) {
                nL =
                    w.container.offsetLeft +
                    (deltaW * (w.container.offsetLeft - area.initLeft)) /
                        area.initWidth;
                nT =
                    w.container.offsetTop +
                    (deltaH * (w.container.offsetTop - area.initTop)) /
                        area.initHeight;
            }
            ...
            if (
                ...
                w.parent.getProp("layout") !== "vertical"
            ) {
                var newWidth = Math.max(nW, GRIDWIDTH) / PXSCALE;
                if (
                    typeof w.props.width === "string" &&
                    w.props.width.indexOf("%") > -1
                ) {
                    w.props.width =
                        (
                            (100 * PXSCALE * newWidth) /
                            w.container.parentNode.offsetWidth
                        )
                            .toFixed(2)
                            .replace(/\.?0+$/, "") + "%";
                } else {
                ...
                if (this.selectedWidgets.length > 1) {
                    ...
                    if (
                        typeof w.props.left === "string" &&
                        w.props.left.indexOf("%") > -1
                    ) {
                        w.props.left =
                            (
                                (100 * PXSCALE * newLeft) /
            ...
            if (
                deltaH !== 0 &&
                w.props.height !== undefined &&
                w.parent.getProp("layout") !== "horizontal"
            ) {
                var newHeight = Math.max(nH, GRIDWIDTH) / PXSCALE;
                if (
                    typeof w.props.height === "string" &&
                    w.props.height.indexOf("%") > -1
                ) {
                    w.props.height =
                        (
                            (100 * PXSCALE * newHeight) /
                            w.container.parentNode.offsetHeight
                        )
                            .toFixed(2)
                            .replace(/\.?0+$/, "") + "%";
                } else {
                ...
                if (this.selectedWidgets.length > 1) {
                    ...
                    if (
                        typeof w.props.top === "string" &&
                        w.props.top.indexOf("%") > -1
                    ) {
                        w.props.top =
                            (
                                (100 * PXSCALE * newTop) /
    ...
    moveWidget(deltaX, deltaY) {
        ...
        for (var w of this.selectedWidgets) {
            var newTop = Math.max(
                parseInt(w.container.offsetTop) / PXSCALE + deltaY,
                0
            );
            if (
                typeof w.props.top === "string" &&
            ...
            var newLeft = Math.max(
                parseInt(w.container.offsetLeft) / PXSCALE + deltaX,
                0
            );
            if (
                typeof w.props.left === "string" &&
    ...
    pushHistory(indexes) {
        ...
        var d = diff.diff(
            this.historySession,
            sessionManager.session.getRoot()
        );
    ...
    updateWidgetFromPatch(patch, indexes) {
        var [widget, subpatch] = diffToWidget(
                widgetManager.getWidgetById("root")[0],
                patch
            ),
            options = {};
        ...
        if (
            ...
            !widgetManager.widgets[this.selectedWidgets[0].hash]
        ) {
            this.unselect();
            this.selectedWidgets = [];
            this.inspector.clear();
