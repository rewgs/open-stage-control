...
var multiSelectKey = (navigator.platform || "").match("Mac")
    ? "metaKey"
    : "ctrlKey";
...
var handleClick = function (event) {
    ...
    if (
        !event.detail[multiSelectKey] &&
        event.type !== "fast-right-click" &&
    ...
    var eventData = event.detail,
        targetWidget = widgetManager.getWidgetByElement(
            eventData.target,
            ":not(.not-editable)"
        );
    ...
    if (
        !targetWidget.container.classList.contains("editing") ||
    ...
    if (event.type !== "fast-right-click") {
        if (targetWidget.getProp("type") === "tab") {
            targetWidget.parent.setValue(
                targetWidget.parent.children.indexOf(targetWidget),
                { sync: true, send: true }
            );
        }
        ...
        if (
            ...
            var siblings = targetWidget.parent.children,
                ...
                range = siblings
                    ...
                    .filter((w) => !editor.selectedWidgets.includes(w));
    ...
    if (
        !event.detail.shiftKey &&
        !event.detail[multiSelectKey] &&
        editor.selectedWidgets.length <= 1
    ) {
    ...
    var clickX =
            Math.round(
                (eventData.offsetX + eventData.target.scrollLeft) /
                    (GRIDWIDTH * PXSCALE)
            ) * GRIDWIDTH,
        clickY =
            Math.round(
                (eventData.offsetY + eventData.target.scrollTop) /
                    (GRIDWIDTH * PXSCALE)
            ) * GRIDWIDTH;
    ...
    if (widget.childrenType !== undefined) {
        if (editor.clipboard !== null) {
            ...
            if (
                (editor.clipboard[0].type === "tab" &&
                    widget.childrenType === "widget") ||
                (editor.clipboard[0].type !== "tab" &&
                    widget.childrenType === "tab")
            )
                canPaste = false;
            ...
            if (canPaste) {
                ...
                if (
                    editor.idClipboard &&
                    widgetManager.getWidgetById(editor.idClipboard).length &&
                    editor.clipboard[0].type !== "tab"
                ) {
                    pasteActions.push({
    ...
    if (
        ...
        editor.selectedWidgets.length === 1 &&
        editor.selectedWidgets[0].parent.children.length > 1
    ) {
        actions.push({
            separator: true
        });
                    ...
                    action: () => {
                        ...
                        if (editor.usePercents) {
                            newData.top =
                                (
                                    (100 * clickY) /
                                    editor.selectedWidgets[0].widget
                                        .offsetHeight
                                ).toFixed(2) + "%";
                            newData.left =
                                (
                                    (100 * clickX) /
                                    editor.selectedWidgets[0].widget.offsetWidth
                                ).toFixed(2) + "%";
                            newData.height = "6%";
                            newData.width = "8%";
                        }
                        ...
                        var indexes = {
                            addedIndexes: [data[0].widgets.length - 1]
                        };
                        ...
                        var newWidget =
                            editor.selectedWidgets[0].children[
                                editor.selectedWidgets[0].children.length - 1
                            ];
                        editor.select([newWidget]);
                        if (!editor.widgetTree.parent.minimized)
        ...
        if (data.length == 1 && widget.childrenType !== "widget") {
            actions.push({
                ...
                action: () => {
                    ...
                    if (!editor.widgetTree.parent.minimized)
                        editor.widgetTree.showWidget(newWidget);
                    newWidget.parent.setValue(
                        newWidget.parent.children.indexOf(newWidget)
                    );
                }
            });
        }
    ...
    if (parent !== widgetManager && data.length === 1) {
        ...
        actions.push({
            label:
                icon("file-download") + " " + locales("editor_fragment_export"),
            action: () => {
                sessionManager =
