        ...
        if (data === null) {
            ...
            for (var converter of converters) {
                if (semver.lte(version, converter.version)) {
                    ...
                    if (converter.widget)
                        this.applyConvert(
                            data.session || data.content,
                            converter.widget
                        );

                    if (converter.warning && type !== "fragment")
                        warning = true;
                }
            }
            ...
            if (warning) {
                new UiModal({
                    title: locales("session_oldversion_title"),
                    content: locales("session_oldversion"),
...
var converters = [
    {
        ...
        widget: (data) => {
            ...
            switch (data.type) {
                ...
                case "matrix":
                    ...
                    if (
                        ...
                        data.matrix.every((x) => typeof x === "number")
                    ) {
                    ...
                    switch (data.widgetType) {
                        ...
                        case "push":
                            data.widgetType = "button";
                            if (oprops)
                                data.props.mode = data.props.norelease
                                    ? "tap"
                                    : "push";
                            break;
                        ...
                        case "fader":
                            if (oprops)
                                data.props.design = data.props.compact
                                    ? "compact"
                                    : "default";
                            break;
                        ...
                        case "strip":
                            ...
                            if (oprops)
                                data.props.layout = data.props.horizontal
                                    ? "horizontal"
                                    : "vertical";
                            break;
                    }
            ...
            if (
                ...
                data.type &&
                data.type &&
                !data.type.match(/button|menu|modal|clone|html|tab/)
            ) {
            ...
            switch (data.type) {
                ...
                case "multixy":
                    if (data.touchAddress && String(data.touchAddress).length) {
                        let sendScript = data.type.match(/range|multixy/)
                                ? "if (touch.length) send(address, touch[0], touch[1])\n else send(address, touch)"
                                : "send(address, touch)",
    ...
    },
    {
        ...
        widget: (data) => {
            if (data.script && String(data.script).includes("toolbar(")) {
                ...
                data.script = String(data.script)
                    .replace("toolbar(4)", "toolbar(5)")
                    .replace("toolbar(3)", "toolbar(4)");
            }
        }
    },
    {
    ...
    },
    {
        ...
        widget: (data) => {
            if (data.type === "patchbay" && data.exclusive === true) {
                ...
                (data.type === "panel" || data.type === "root") &&
                data.verticalTabs === true
            ) {
