...
class Settings {
    constructor() {
        ...
        this.container = DOM.get("#osc-container")[0].appendChild(
            html`<osc-settings></osc-settings>`
        );
        this.names = [];
        this.options = {};
    ...
    saveAs() {
        dialog
            .showSaveDialog({
                title: "Open File",
                filters: [
                    { name: "OSC Config (.config)", extensions: ["config"] }
                ],
                properties: ["openFile"]
            })
            .then((file) => {
                ...
                if (this.configPath.split(".").pop() !== "config")
        ...
        dialog
            ...
            .then((file) => {
                ...
                this.options = JSON.parse(
                    fs.readFileSync(file.filePaths[0], "utf-8")
                );
                this.create();
                this.write();
    ...
    create() {
        ...
        for (let k in settings.options) {
            ...
            let value =
                    this.options[name] === undefined ? "" : this.options[name],
            ...
            if (data.type === "boolean") {
                toggle = field.appendChild(html`
                    <osc-settings-checkbox class="${value ? "on" : ""}"
                        >${raw(icon("check"))}</osc-settings-checkbox
                    >
                `);
            ...
            if (data.file) {
                browse = field.appendChild(html`
                    <osc-settings-file
                        >${raw(icon("folder-open"))}</osc-settings-file
                    >
                `);
            ...
            input.addEventListener("change", (e) => {
                ...
                if (
                    ...
                    (v !== "" &&
                        data.check &&
                        data.check(v, this.options) !== true)
                ) {
                ...
                if (
                    data.restart &&
                    !field.classList.contains("restart") &&
                    v != value
                ) {
                    field.classList.add("restart");
                    field.classList.add("warning");
                    terminal.log(
                        `(WARNING) --${name}: The launcher must be restarted for this change to take effect.`,
                        "warning"
                    );
                } else if (
                    data.restart &&
